...
int sext(int num, int size){
    switch (size){
        case 5:
            if(num & 0x10) return num | 0xFFE0;
            else return num;

        case 9:
            if(num & 0x100) return num | 0xFE00;
            else return num;

        case 11:
            if(num & 0x400) return num | 0xF800;
            else return num;

        case 6:
            if(num & 0x20) return num | 0xFFC0;
            else return num;
        
        case 8:
            if(num & 0x80) return num | 0xFF00;
            else return num;
...
void welogic(int addy0, int rw, int datasize, int* we0, int* we1){
    if(rw == 1){                //1 = write
        if(datasize == 0){      //0 = byte
            if(addy0){
                *we1 = 1;
                *we0 = 0;
            }
...
int dcash_res_logic(int dcache_res, int datasize, int addy0){
    if(datasize == 1) return dcache_res;
    else if(datasize == 0){
        if(addy0 == 0) return sext(dcache_res & 0xFF, 8);
        else if(addy0 == 1){
            int res = (dcache_res & 0xFF00) >> 8;
            res = res & 0xFF;
            return sext(res, 8);
        }
        else return 0;
...
int dcash_feed_logic(int input, int datasize, int addy0){
    if(datasize == 1) return input;
    else if(datasize == 0) {
        if(addy0 == 0) return input;
        else if(addy0 == 1) return Low16bits(input << 8);
        else return 0;
    }
    else return 0;
}

int brlogic(int memv, int brop, int uncon, int trop, int memcc, int ir11_9){
    int real_n = (memcc & 0x4) >> 2;
    int real_z = (memcc & 0x2) >> 1;
    int real_p = memcc & 0x1;
    int inst_n = (ir11_9 & 0x4) >> 2;
    int inst_z = (ir11_9 & 0x2) >> 1;
    int inst_p = ir11_9 & 0x1;
    if(memv){
        if(brop){
            if((real_n && inst_n) || (real_z && inst_z) || (real_p && inst_p)) return 1;
            else return 0;
        }
        else if(uncon) return 1;
        else if(trop) return 2;
        else return 0;
    }
    else return 0;
}

int alu(int signal, int one, int two){
    if(signal == 0){
        return Low16bits(one + two);
    }
    else if(signal == 1){
        return Low16bits(one & two);
    }
        ...
        return Low16bits(two);
    }
    else return 0;
}

int shf(int signal, int in){
    int bit5 = signal & 0x20;
    int bit4 = signal & 0x10;
    int amount = signal & 0xF;
    int value;
    if(bit4){
        if(bit5){
            int bit15 = Low16bits(in) & 0x8000;
            value = in;
        ...
        else return Low16bits(in >> amount);
    }
    else return Low16bits(in << amount);
}
...
int mem_drid, mem_pcmux, v_mem_ld_cc, v_mem_ld_reg, target_pc, trap_pc;  //globals or outgoing values to other stages

void stalllogic(int ldcc, int ldrg, int brstall, int v, int* vldcc, int* vldreg, int* vbrstall){                         //function that uses above defined globals
    if(v){
        *vldcc = ldcc;
        *vldreg = ldrg;
        *vbrstall = brstall;
...
void MEM_stage() {
    ...
    int dcache_en, dcache_rw, data_size, br_op, uncond_op, trap_op, br_stall;
    br_stall = Get_MEM_BR_STALL(PS.MEM_CS);             //value retrieval
    dcache_en = Get_DCACHE_EN(PS.MEM_CS);
    ...
    int dcache_ready = 0;                               //inits
    int r_word = 0x0000;
    int w_word;
    int mem_w0, mem_w1;


    welogic((PS.MEM_ADDRESS & 0x1), dcache_rw, data_size, &mem_w0, &mem_w1);            //dcache access
    w_word = dcash_feed_logic(PS.MEM_ALU_RESULT, data_size, (PS.MEM_ADDRESS & 0x1));

    if(dcache_en && PS.MEM_V) dcache_access(PS.MEM_ADDRESS, &r_word, w_word, &dcache_ready, mem_w0, mem_w1);

    mem_stall = ~dcache_ready & (dcache_en & PS.MEM_V);
    NEW_PS.SR_DATA = trap_pc = dcash_res_logic(r_word, data_size, (PS.MEM_ADDRESS & 0x1));
    target_pc = PS.MEM_ADDRESS;
    


    mem_pcmux = brlogic(PS.MEM_V, br_op, uncond_op, trap_op, PS.MEM_CC, (PS.MEM_IR & 0xE00) >> 9);          //bottom left
    mem_drid = PS.MEM_DRID;

    stalllogic(Get_MEM_LD_CC(PS.MEM_CS), Get_MEM_LD_REG(PS.MEM_CS), Get_MEM_BR_STALL(PS.MEM_CS), PS.MEM_V, &v_mem_ld_cc, &v_mem_ld_reg, &v_mem_br_stall);     //bottom middle and right
    if(mem_stall) NEW_PS.SR_V = 0;
    else NEW_PS.SR_V = PS.MEM_V;

    NEW_PS.SR_ADDRESS = PS.MEM_ADDRESS;                 //all other latches
...
void AGEX_stage() {
    ...
    int adder_in1, adder_in2, addr2res, mem_addy;
                                                                                        
    if(Get_ADDR1MUX(PS.AGEX_CS)) adder_in1 = PS.AGEX_SR1;
    else adder_in1 = PS.AGEX_NPC;

    if(Get_ADDR2MUX(PS.AGEX_CS) == 0) addr2res = 0;
    else if(Get_ADDR2MUX(PS.AGEX_CS) == 1) addr2res = sext(PS.AGEX_IR & 0x3F, 6);
    else if(Get_ADDR2MUX(PS.AGEX_CS) == 2) addr2res = sext(PS.AGEX_IR & 0x1FF, 9);
    else if(Get_ADDR2MUX(PS.AGEX_CS) == 3) addr2res = sext(PS.AGEX_IR & 0x7FF, 11);
    else addr2res = 0;

    if(Get_LSHF1(PS.AGEX_CS)) adder_in2 = Low16bits(addr2res << 1);
    else adder_in2 = addr2res;

    if(Get_ADDRESSMUX(PS.AGEX_CS)) mem_addy = Low16bits(adder_in1 + adder_in2);
    else mem_addy = Low16bits((PS.AGEX_IR & 0xFF) << 1);


    ////////////////////alu and shf
    int alu_b, shf_res, alu_res, alushf_res;

    if(Get_SR2MUX(PS.AGEX_CS)) alu_b = sext(PS.AGEX_IR & 0x1F, 5);
    else alu_b = PS.AGEX_SR2;

    alu_res = alu(Get_ALUK(PS.AGEX_CS), PS.AGEX_SR1, alu_b);
    shf_res = shf(PS.AGEX_IR & 0x3F, PS.AGEX_SR1);

    if(Get_ALU_RESULTMUX(PS.AGEX_CS)) alushf_res = alu_res;
    else alushf_res = shf_res;


    //////////////////logic blocks
    int valid;
    stalllogic(Get_AGEX_LD_CC(PS.AGEX_CS), Get_AGEX_LD_REG(PS.AGEX_CS), Get_AGEX_BR_STALL(PS.AGEX_CS), PS.AGEX_V, &v_agex_ld_cc, &v_agex_ld_reg, &v_agex_br_stall);
    ...
    /* Your code for latching into MEM latches goes here */
        NEW_PS.MEM_ADDRESS = mem_addy;
        NEW_PS.MEM_NPC = PS.AGEX_NPC;
        NEW_PS.MEM_CC = PS.AGEX_CC;
        NEW_PS.MEM_ALU_RESULT = alushf_res;
...
int dependencylogic(int v, int sr2, int sr1, int sr1_need, int sr2_need, int brop){
    //printf("%x, %x, %x, %x, %x, %x\n", v, sr2, sr1, sr1_need, sr2_need, brop);
    //printf("%x, %x, %x, %x, %x, %x\n", v_agex_ld_reg, v_mem_ld_reg, v_sr_ld_reg, v_agex_ld_cc, v_mem_ld_cc, v_sr_ld_cc);
    //printf("%x, %x, %x,\n", sr_reg_id, mem_drid, agex_drid);
    /*
    if(v){
        if(v_sr_ld_reg){
            if(((sr_reg_id == sr2) && sr2_need) || ((sr_reg_id == sr1) && sr1_need)) dep_stall = 1;
            else dep_stall = 0;
        }
        if(v_mem_ld_reg){
            if(((mem_drid == sr2) && sr2_need) || ((mem_drid == sr1) && sr1_need)) dep_stall = 1;
            else dep_stall = 0;
        }
        if(v_agex_ld_reg){
            if(((agex_drid == sr2) && sr2_need) || ((agex_drid == sr1) && sr1_need)) dep_stall = 1;
            else dep_stall = 0;
        }
        ...
        if(brop){
            ...
            if(!v_agex_ld_reg && !v_mem_ld_reg && !v_sr_ld_reg && !v_agex_ld_cc && !v_mem_ld_cc && !v_sr_ld_cc) dep_stall = 0;
        }  
    }
    else dep_stall = 0;
    printf("%x\n", dep_stall);
    */
    ...
    if(v){
        if(brop){
            if(v_agex_ld_cc || v_mem_ld_cc || v_sr_ld_cc) return 1;
            else return 0;
        }
        else{
            if(v_agex_ld_reg && (((agex_drid == sr2) && sr2_need) || ((agex_drid == sr1) && sr1_need))) return 1;
            else if(v_mem_ld_reg && (((mem_drid == sr2) && sr2_need) || ((mem_drid == sr1) && sr1_need))) return 1;
            else if(v_sr_ld_reg && (((sr_reg_id == sr2) && sr2_need) || ((sr_reg_id == sr1) && sr1_need))) return 1;
            else return 0;
        }
    }
    else return 0;
...
void DE_stage() {
    ...
    ///////////////////////control store and br stall
    CONTROL_STORE_ADDRESS = (((PS.DE_IR >> 11) << 1) | (PS.DE_IR & 0x20) >> 5);


    //////////////////////reg file
    int sr1_in, sr2_in, sr2_res, sr1_res, cc_res;
    sr1_in = (PS.DE_IR & 0x1C0) >> 6;
    if(PS.DE_IR & 0x2000) sr2_in = (PS.DE_IR & 0xE00) >> 9;
    else sr2_in = PS.DE_IR & 0x7;
    ...
    ///////////////////br stall
    if(PS.DE_V) v_de_br_stall = CONTROL_STORE[CONTROL_STORE_ADDRESS][15]; //br.stall signal
    else v_de_br_stall = 0;

    //////////////////dependency
    dep_stall = dependencylogic(PS.DE_V, sr2_in, sr1_in, 
        CONTROL_STORE[CONTROL_STORE_ADDRESS][0], CONTROL_STORE[CONTROL_STORE_ADDRESS][1], CONTROL_STORE[CONTROL_STORE_ADDRESS][12]);
    ...
    if(mem_stall) LD_AGEX = 0;
        ...
        LD_AGEX = 1;
        if(dep_stall) ld_agex_v = 0;
        else ld_agex_v = PS.DE_V;
    }
    ...
    /* Your code for latching into AGEX latches goes here */
        ...
        NEW_PS.AGEX_SR1 = sr1_res;
        NEW_PS.AGEX_SR2 = sr2_res;
    ...
    /////////////////// this piece down here for register timing issues
    /////////////////// wouldn't be a problem with non-blocking verilog, need old CC values for above assignment
    if(v_sr_ld_cc) {
        N = sr_n;
        Z = sr_z;
        P = sr_p;
...
void FETCH_stage() {
    ...
    int npc = PC + 2;
    int ldde, ldv, ldpc, pc_res;
    /* your code for FETCH stage goes here */

    int icache_res;
    icache_access(Low16bits(PC), &icache_res, &icache_r);

    ldpc = 0;
    if(!mem_stall && !dep_stall && !v_agex_br_stall && !v_de_br_stall && !v_mem_br_stall && icache_r) ldpc = 1;
    if(!mem_stall && !dep_stall && !v_agex_br_stall && !v_de_br_stall && v_mem_br_stall && mem_pcmux) ldpc = 1;     
    /////////////////// stallin
    if(mem_stall||dep_stall) ldde = 0;
    else{
        ldde = 1;
        if(v_agex_br_stall || v_mem_br_stall || v_de_br_stall || !icache_r) ldv = 0;
        else {
            ldv = 1;
        }
    }
