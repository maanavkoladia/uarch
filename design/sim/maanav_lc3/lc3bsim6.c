...
int16_t SEXT(int16_t value, int width);
//mem globs
#define GET_BIT(data, bit) ((data >> bit) & (0x0001))
#define APPLY_MASK(data, mask) (data & mask)
...
int dcache_r = 0;
int mem_drid = 0;
void Load_Logic_Block(uint16_t data){

    int dataSize = Get_DATA_SIZE(PS.MEM_CS);
    int mem_address = (PS.MEM_ADDRESS);

    if(dataSize){
        NEW_PS.SR_DATA = APPLY_MASK(data, 0xFFFF);
    }else{
        if(APPLY_MASK(mem_address, 0x01)){//need to shitf down and sext
            NEW_PS.SR_DATA = (uint16_t)SEXT(APPLY_MASK(data >> 8, 0xFF), 8);
        }else{//pull out lower bytes
            NEW_PS.SR_DATA = (uint16_t)SEXT(APPLY_MASK(data, 0xFF), 8);
        }
    }
    trapPC = NEW_PS.SR_DATA;
    //LOG("reached end of Load_Logic_Block");
}

uint16_t Write_Logic_Block(){ 
    int dataSize = Get_DATA_SIZE(PS.MEM_CS);
    int mem_address = (PS.MEM_ADDRESS);

    if(dataSize){
        return PS.MEM_ALU_RESULT;
    }else{//now dealing with btye rw
        if(mem_address & 0x1){//using upper btye
            return APPLY_MASK((PS.MEM_ALU_RESULT<< 8), 0xFF00);//pull out upper 8 btyes
        }else{
            return APPLY_MASK(PS.MEM_ALU_RESULT, 0xFF);
        }
    }
    LOG("reached end of writie logic block");
}

void We_logic_Block(int* we1, int* we0){
    *we0 = 0;
    *we1 = 0;
    
    int dataSize = Get_DATA_SIZE(PS.MEM_CS);
    int mem_address = (PS.MEM_ADDRESS);
    int dcacheRW = Get_DCACHE_RW(PS.MEM_CS);

    if (dcacheRW == 0) {//doing a read
        return;
    }

    if (dataSize == 0) {
        if (mem_address & 0x1) {//unalgned
            *we0 = 0;
            *we1 = 1;
        } else {//aligned 
            *we0 = 1;
...
void computeMemStall(void){
    int dcache_en = Get_DCACHE_EN(PS.MEM_CS);
    int memV = PS.MEM_V;
    int dcache_r_local = dcache_r;

    mem_stall = (dcache_en && memV && !dcache_r_local)?1:0;
}

void Br_Logic_Block(){
    int memV = PS.MEM_V;
    int memCC = APPLY_MASK(PS.MEM_CC, 0x07);
    int IR_9_11 = (PS.MEM_IR & 0x0E00) >> 9;
    int brOp = Get_BR_OP(PS.MEM_CS);
    int unconOp = Get_UNCOND_OP(PS.MEM_CS);
    int trapOp = Get_TRAP_OP(PS.MEM_CS);
    int ben = memCC & IR_9_11;
    ...
    if(debugFlag > 1){LOG("multiple condtions entered");}
    if(memPCMux > 2 || memPCMux < 0){LOG("invalid pcmux going out of logic block");}
}

void mem_bkwrds_logic_block(void){
    if(PS.MEM_V){
...
void MEM_stage() {
    ...
    int we1, we0;
    uint16_t readData_buf = 0;
    We_logic_Block(&we1, &we0); 
    int writeData = Write_Logic_Block();

    if(Get_DCACHE_EN(PS.MEM_CS) && PS.MEM_V){
        dcache_access(PS.MEM_ADDRESS, (int*)&readData_buf, writeData, &dcache_r, we0, we1);
    }

    Load_Logic_Block(readData_buf);
    targetPC = PS.MEM_ADDRESS;
    computeMemStall();
    mem_bkwrds_logic_block();
    ld_sr_v_logicBlock();
...
int v_agex_ld_cc = 0;
int agex_drid = 0;
int16_t addr1mux(uint16_t agexnpc, uint16_t agex_sr1){
...
int16_t ADDR2MUX_HW(int16_t agexIR){
    int ADDR2MUX_select = Get_ADDR2MUX(PS.AGEX_CS);
    if(ADDR2MUX_select == 0){ 
        return 0;
    }else if (ADDR2MUX_select == 1){
        return SEXT(APPLY_MASK(agexIR, 0x3F), 6);
    }else if (ADDR2MUX_select == 2){
        return SEXT(APPLY_MASK(agexIR, 0x01FF), 9);
    }    
    return SEXT(APPLY_MASK(agexIR, 0x07FF), 11);
}

int16_t ZEXT_LSHF1(uint8_t ir) {
    uint16_t zext = (uint16_t)(ir); // Zero-extend 8-bit IR to 16-bit
    return (int16_t)(zext << 1); // Left shift by 1
}

int16_t ADDER_HW(int16_t inFromADDR2Mux, int16_t inFromADDR1MUX){
    return inFromADDR2Mux + inFromADDR1MUX;
}

uint16_t addressmux(int16_t addrout, int zextout){
...
int16_t LSHF_HW(int16_t in){
    int lsfh_signal = Get_LSHF1(PS.AGEX_CS);
...
uint16_t agex_computeMemAddress(void){
    int16_t addr1muxout = addr1mux(PS.AGEX_NPC, PS.AGEX_SR1);
    int16_t addr2mux_out = ADDR2MUX_HW(PS.AGEX_IR);
    int16_t lshf = LSHF_HW(addr2mux_out);
    int16_t adderout = ADDER_HW(addr1muxout, lshf);
    int16_t zextout = ZEXT_LSHF1(APPLY_MASK(PS.AGEX_IR, 0xFF));
    int16_t addressmux_out = addressmux(adderout, zextout);
    return addressmux_out;
}

int16_t ALUK_HW(int16_t A, int16_t B){
    int ALUK_Select = Get_ALUK(PS.AGEX_CS);
    
    if(ALUK_Select == 0){
        return A + B;
    }else if (ALUK_Select == 1){
        return A & B;
    }else if (ALUK_Select == 2) {
        return A ^ B;
    }else{
        return B;
    }
...
int16_t SHF_HW(int16_t src1, int A, int D, int16_t amt4) {
    if (A && D) { // Arithmetic shift right
        return (int16_t)(src1 >> amt4); // Preserves sign bit
    } else if (!A && D) { // Logical shift right
        return (int16_t)((uint16_t)src1 >> amt4); // Treat as unsigned to shift in 0s
    } else if(!A && !D){ // Logical shift left
        return src1 << amt4;
    }else{
        return src1;
    }
}
int16_t sr2_mux(void){
    if(Get_SR2MUX(PS.AGEX_CS)){
        return SEXT(APPLY_MASK(PS.AGEX_IR, 0x1F), 5);
    }else{
        return PS.AGEX_SR2;
    }
}
int16_t aluresultmux_hw(int16_t shf_in, int16_t alu_in){
...
int16_t computeALUREs(void){
    int16_t sr2 = sr2_mux();    
    int16_t alu_out = ALUK_HW(PS.AGEX_SR1, sr2);
    int16_t shf_out = SHF_HW(PS.AGEX_SR1, GET_BIT(PS.AGEX_IR, 5), GET_BIT(PS.AGEX_IR, 4), APPLY_MASK(PS.AGEX_IR, 0xF));
    int16_t alures_out =  aluresultmux_hw(shf_out, alu_out);
    return alures_out;
}

void agex_Backwrds_LogicBlock(void){
    if(PS.AGEX_V){
...
void AGEX_stage() {
    ...
    compute_ldMem(&LD_MEM);
    agex_Backwrds_LogicBlock();
    
    if (LD_MEM) {
        /* Your code for latching into MEM latches goes here */
        //latch others
        NEW_PS.MEM_ADDRESS = (uint16_t)agex_computeMemAddress();
        NEW_PS.MEM_ALU_RESULT = (uint16_t)computeALUREs();
...
//int uinstr[NUM_CONTROL_STORE_BITS];
int addr_glob = 0;
void depchecklogicblock(int sr1, int sr2){
    int flag = 0;
    int sr1needed = Get_SR1_NEEDED(CONTROL_STORE[addr_glob]);
    int sr2needed = Get_SR2_NEEDED(CONTROL_STORE[addr_glob]);
    int brOp = Get_DE_BR_OP(CONTROL_STORE[addr_glob]);
    ...
    if (v_agex_ld_reg) {
        if (sr1needed && (agex_drid == sr1)) {
            LOG("Dependency stall: AGEX stage is writing SR1 (reg %d)", sr1);
            flag = 1;
        } 
        if (sr2needed && (agex_drid == sr2)) {
            LOG("Dependency stall: AGEX stage is writing SR2 (reg %d)", sr2);
            flag = 1;
        }
    }

    if (v_mem_ld_reg) {
        if (sr1needed && (mem_drid == sr1)) {
            LOG("Dependency stall: MEM stage is writing SR1 (reg %d)", sr1);
            flag = 1;
        } 
        if (sr2needed && (mem_drid == sr2)) {
            LOG("Dependency stall: MEM stage is writing SR2 (reg %d)", sr2);
            flag = 1;
        }
    }

    if (v_sr_ld_reg) {
        if (sr1needed && (sr_reg_id == sr1)) {
            LOG("Dependency stall: SR stage is writing SR1 (reg %d)", sr1);
            flag = 1;
        } else if (sr2needed && (sr_reg_id == sr2)) {
            LOG("Dependency stall: SR stage is writing SR2 (reg %d)", sr2);
            flag = 1;
        }
    }

    if (brOp && (v_agex_ld_cc || v_mem_ld_cc || v_sr_ld_cc)) {
        LOG("Dependency stall: Branch operation depends on condition codes (CC)");
        flag = 1;
    }
...
void br_stall_logic_block(void){
    if(PS.DE_V && Get_DE_BR_STALL(CONTROL_STORE[addr_glob])){
        v_de_br_stall = 1;
    }else{
        v_de_br_stall = 0;
    }
...
void ld_agex_logic_block(int* ldagex){
    if(mem_stall){//highest pri
        *ldagex = 0;
        return;
    }
...
void getControlStroreAddr(int* addr_buf){
    uint16_t ir = APPLY_MASK(PS.DE_IR, 0xFFFF);
    *addr_buf = (APPLY_MASK(ir, 0xF000) >> 10) | (APPLY_MASK(ir, 0x0800) >> 10) | (APPLY_MASK(ir, 0x0020) >> 5);
}

void Get_sr1_and_sr2(int* sr1, int* sr2){ 
    int sr2_id = GET_BIT(PS.DE_IR, 13);
    *sr1 = (APPLY_MASK(PS.DE_IR, 0x01C0)) >> 6;
    if(sr2_id){
        *sr2 = (APPLY_MASK(PS.DE_IR, 0x0E00)) >> 9;
    }else{
        *sr2 = (APPLY_MASK(PS.DE_IR, 0x0007));
    }
}

void ld_regFile(void){
    if(v_sr_ld_reg){
...
void DE_stage() {
    ...
    int sr1, sr2;
    int savednzp = APPLY_MASK(((N << 2) | (Z << 1) | (P)), 0x07);
    Get_sr1_and_sr2(&sr1, &sr2);
    int sr1data = REGS[sr1];
    int sr2data = REGS[sr2];
    getControlStroreAddr(&CONTROL_STORE_ADDRESS);    
    addr_glob = CONTROL_STORE_ADDRESS;
    depchecklogicblock(sr1, sr2);
    br_stall_logic_block();
    ld_agex_logic_block(&LD_AGEX);
    ld_regFile();
    ld_cc();
    ...
    if (LD_AGEX) {
        ...
        NEW_PS.AGEX_SR1 = sr1data;
        NEW_PS.AGEX_SR2 = sr2data; 
        NEW_PS.AGEX_CC = savednzp; 
        NEW_PS.AGEX_DRID = Get_DRMUX(CONTROL_STORE[addr_glob])? 7 : APPLY_MASK((PS.DE_IR >> 9), 0x7);
        //v bit done in logib lbock
...
int ld_pc_logic_block(void){
    if ((!(v_de_br_stall || v_agex_br_stall || v_mem_br_stall || dep_stall || mem_stall)  && icache_r) || (v_mem_br_stall && memPCMux)) {
        return 1;
    }
    return 0;
}

void ld_de_bit_logic_block(int* ld_de){
    if(mem_stall || dep_stall){
        *ld_de = 0;
    }else if(v_de_br_stall || v_agex_br_stall || v_mem_br_stall || !icache_r){
        *ld_de = 1;
        NEW_PS.DE_V = 0;
    }else{
        *ld_de = 1;
    ...
    //LOG("LEFT LOGIC bloc somehow");
}

void FETCH_stage() { 
    LOG("Fetch running");
    int instr_buf;
    icache_access(PC, &instr_buf, &icache_r);
    int ld_pc = ld_pc_logic_block();
    int ld_de;
    ld_de_bit_logic_block(&ld_de);
    int savedPC = PC;
    if(ld_pc){
        switch (memPCMux) {
            case 0:
    ...
    if(ld_de){
        NEW_PS.DE_IR = instr_buf;
        NEW_PS.DE_NPC = savedPC + 2;
        //de_V done in ld v logic block
    }
}

int16_t SEXT(int16_t value, int width) {
    if (width <= 0 || width > 16) {
        return value; // Invalid bit width, return unchanged
    }

    int16_t sign_bit = 1 << (width - 1); // Calculate the sign bit based on the width

    if (value & sign_bit) {
        // If the sign bit is set, extend the sign
        value |= ~((1 << width) - 1); // Set the upper bits to 1
    } else {
        // If the sign bit is not set, mask out any upper bits
        value &= (1 << width) - 1; // Clear the upper bits
    }
