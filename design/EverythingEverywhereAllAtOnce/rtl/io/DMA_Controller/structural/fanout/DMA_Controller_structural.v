...
module DMA_Controller (
    ...
    // Each valid_mask[i] had fanout 8 (8 AND_2 cells in g_wb_byte[i]).
    // Buffer per bit with bufferH16$. +0.24 ns on valid_mask path. Each
    // tcoded[i] still feeds 1 OR_2; large_diff still feeds 16, no cascade.
    wire [15:0] valid_mask;
    wire [15:0] valid_mask_pre;
    genvar vi;
    generate
        for (vi = 0; vi < 16; vi = vi + 1) begin : g_vmask
            `OR_2(u_vm, 1, valid_mask_pre[vi], tcoded[vi], large_diff)
            bufferH16$ u_vm_buf (.out(valid_mask[vi]), .in(valid_mask_pre[vi]));
        end
    endgenerate
