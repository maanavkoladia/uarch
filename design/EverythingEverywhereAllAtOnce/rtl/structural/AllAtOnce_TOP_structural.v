...
module AllAtOnce_TOP (
    ...
    generate
        for (gp_i = 0; gp_i < 4; gp_i = gp_i + 1) begin : g_core_stq_data_pack
            for (gp_b = 0; gp_b < CACHE_LINES_SIZE_B; gp_b = gp_b + 1) begin : g_byte
                assign core_stq_data_flat[gp_i][gp_b*8 +: 8] =
                    core2dcache.stq_heads[gp_i].data[gp_b];
            end
        end
        for (gp_b = 0; gp_b < CACHE_LINES_SIZE_B; gp_b = gp_b + 1) begin : g_core_stq_mio_pack
            assign core_stq_info_mio_data_flat[gp_b*8 +: 8] =
                core2dcache.stq_info_mio.data[gp_b];
        end
    endgenerate
    ...
    generate
        for (gp_b = 0; gp_b < CACHE_LINES_SIZE_B; gp_b = gp_b + 1) begin : g_icache_line_unpack
            assign icache2core.instruction_line[gp_b] =
                icache_instruction_line_w[gp_b*8 +: 8];
        end
    endgenerate
    ...
    generate
        ...
        for (gp_b = 0; gp_b < CACHE_LINES_SIZE_B; gp_b = gp_b + 1) begin : g_dcache_mio_line_unpack
            assign dcache2core.line_MIO[gp_b] =
                dcache_out2Core_line_MIO_w[gp_b*8 +: 8];
        end
    endgenerate
