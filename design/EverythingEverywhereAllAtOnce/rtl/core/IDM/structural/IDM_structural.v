// =============================================================================
// IDM (pure Verilog-2005, flat ports)
//
//   Reference SV port (kept for documentation):
//     module IDM (
//         input  fetch_outputs_t fetch_outs_i,
//         output idm_outputs_t   idm_outs_o
//     );
//
//   Only the following fields of fetch_outputs_t are consumed:
//     - exp_pipe_clear
//     - idm_reqs.req[0..3].{ld_meta_data, ld_data, valid, br_valid,
//                           br_eip, br_target, br_xcl, data[CACHE_LINES_SIZE_B]}
//
//   Output idm_outputs_t produced:
//     - valid_slots [2:0]
//     - idm_slots[0..3].{valid, br_valid, br_eip, br_btb_target, br_xcl,
//                        data[CACHE_LINES_SIZE_B]}
//
//   Each `data` byte_t[16] is exposed as a single 128-bit packed bus on the
//   boundary (byte 0 = bits [7:0], matching the SV unpacked-array byte order).
//
//   Reset model:
...
module IDM (
    ...
    // ----------------------------------------------------------------
    // Per-slot internal arrays mirror the original IDM_structural.sv
    // body so the generate loops below stay structurally identical.
    // ----------------------------------------------------------------
    ...
    // ---- bind flat input ports into the internal per-slot arrays ----
    ...
    // ----------------------------------------------------------------
    // Drive flat output ports from the per-slot register arrays.
    // ----------------------------------------------------------------
