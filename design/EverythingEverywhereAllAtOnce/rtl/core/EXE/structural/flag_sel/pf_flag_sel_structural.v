// Structural Verilog 2005 port of EXE/flag_sel/pf_flag_sel.sv
// 10-source op_type-driven PF flag selector.
...
module pf_flag_sel (
    ...
    wire tristated_bus;
    `TRISTATE_L(u_tri_adc,     1, enbar_adc,     adc_pf,     tristated_bus)
    `TRISTATE_L(u_tri_add,     1, enbar_add,     add_pf,     tristated_bus)
    `TRISTATE_L(u_tri_and,     1, enbar_and,     and_pf,     tristated_bus)
    `TRISTATE_L(u_tri_cmp,     1, enbar_cmp,     cmp_pf,     tristated_bus)
    `TRISTATE_L(u_tri_cmpxchg, 1, enbar_cmpxchg, cmpxchg_pf, tristated_bus)
    `TRISTATE_L(u_tri_or,      1, enbar_or,      or_pf,      tristated_bus)
    `TRISTATE_L(u_tri_sal,     1, enbar_sal,     sal_pf,     tristated_bus)
    `TRISTATE_L(u_tri_sar,     1, enbar_sar,     sar_pf,     tristated_bus)
    `TRISTATE_L(u_tri_sbb,     1, enbar_sbb,     sbb_pf,     tristated_bus)
    `TRISTATE_L(u_tri_iretd,   1, enbar_iretd,   iretd_pf,   tristated_bus)

    `MUX_2(u_mux_pf_o, 1, pf_flag_o, curr_pf_flag, tristated_bus, match_any)
