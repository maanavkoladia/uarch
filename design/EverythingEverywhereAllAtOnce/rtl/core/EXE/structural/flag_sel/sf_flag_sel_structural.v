// Structural Verilog 2005 port of EXE/flag_sel/sf_flag_sel.sv
...
module sf_flag_sel (
    ...
    wire tristated_bus;
    `TRISTATE_L(u_tri_adc,     1, enbar_adc,     adc_sf,     tristated_bus)
    `TRISTATE_L(u_tri_add,     1, enbar_add,     add_sf,     tristated_bus)
    `TRISTATE_L(u_tri_and,     1, enbar_and,     and_sf,     tristated_bus)
    `TRISTATE_L(u_tri_cmp,     1, enbar_cmp,     cmp_sf,     tristated_bus)
    `TRISTATE_L(u_tri_cmpxchg, 1, enbar_cmpxchg, cmpxchg_sf, tristated_bus)
    `TRISTATE_L(u_tri_or,      1, enbar_or,      or_sf,      tristated_bus)
    `TRISTATE_L(u_tri_sal,     1, enbar_sal,     sal_sf,     tristated_bus)
    `TRISTATE_L(u_tri_sar,     1, enbar_sar,     sar_sf,     tristated_bus)
    `TRISTATE_L(u_tri_sbb,     1, enbar_sbb,     sbb_sf,     tristated_bus)
    `TRISTATE_L(u_tri_iretd,   1, enbar_iretd,   iretd_sf,   tristated_bus)

    `MUX_2(u_mux_sf_o, 1, sf_flag_o, curr_sf_flag, tristated_bus, match_any)
