// Structural Verilog 2005 port of EXE/FunctionalUnits/and_op.sv
// merged_result = per-byte mux of AND(srA, srB) vs srA under data_size mask;
//                 high 32 bits forced to 0.
// dr_o = res_buf_o = merged_result.
// CF = OF = AF = 0.
// PF = ~^merged[7:0]   (data_size-independent in SV ref).
...
module and_op (
    ...
    `AND_2(u_and_full, 64, and_result, srA, srB)

    wire [7:0]  m_b0, m_b1;
    wire [7:0]  m_b0_raw, m_b1_raw;
    wire [15:0] m_hi;
    `MUX_2(u_mux_b0, 8,  m_b0_raw, srA[7:0],   and_result[7:0],   data_size[0])
    `MUX_2(u_mux_b1, 8,  m_b1_raw, srA[15:8],  and_result[15:8],  data_size[1])
    `MUX_2(u_mux_hi, 16, m_hi,     srA[31:16], and_result[31:16], data_size[2])

    // Buffer m_b0/m_b1 with bufferH16$ (worst-bit fanout 5, single-stage).
    genvar gi_mb;
    generate
    ...
    // PF: parity of merged[7:0] regardless of data_size.
    pf_red_8 u_pf (.x(merged[7:0]), .p(PF));
