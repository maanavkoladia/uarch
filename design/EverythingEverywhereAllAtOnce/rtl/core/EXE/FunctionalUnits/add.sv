// ADD - Addition Functional Unit
// Handles ADD8, ADD16, ADD16sign, ADD32, ADD32sign
...
module add (
    ...
    always_comb begin
        ...
        case (data_size)
            2'b00: begin  // 8-bit ADD
                sum8 = operand1[7:0] + operand2[7:0];
                result = {56'h0, sum8[7:0]};
                
                CF = sum8[8];
                ZF = (sum8[7:0] == 8'h0);
                SF = sum8[7];
                PF = ~^sum8[7:0];  // Even parity
                AF = ((operand1[3:0] + operand2[3:0]) > 4'hF);
                OF = (operand1[7] == operand2[7]) && (operand1[7] != sum8[7]);
            end
            
            2'b01: begin  // 16-bit ADD
                sum16 = operand1[15:0] + operand2[15:0];
                result = {48'h0, sum16[15:0]};
                
                CF = sum16[16];
                ZF = (sum16[15:0] == 16'h0);
                SF = sum16[15];
                PF = ~^sum16[7:0];  // Even parity on low byte
                AF = ((operand1[3:0] + operand2[3:0]) > 4'hF);
                OF = (operand1[15] == operand2[15]) && (operand1[15] != sum16[15]);
            end
            
            2'b10: begin  // 32-bit ADD
                sum32 = operand1[31:0] + operand2[31:0];
                result = {32'h0, sum32[31:0]};
                
                CF = sum32[32];
                ZF = (sum32[31:0] == 32'h0);
                SF = sum32[31];
                PF = ~^sum32[7:0];  // Even parity on low byte
                AF = ((operand1[3:0] + operand2[3:0]) > 4'hF);
