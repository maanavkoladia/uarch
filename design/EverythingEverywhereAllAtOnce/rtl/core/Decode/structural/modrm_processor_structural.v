...
module modrm_processor (
    ...
    output wire                    st_op,
    output wire                    ld_op,
    // fanout-split duplicates: parallel AND_2 drivers wired to a disjoint
    // subset of the original consumers in the parent so each leaf cell stays
    // below the structural fanout limit without inserting a buffer.
    output wire                    st_op_dup,
    output wire                    ld_op_dup,
    output wire                    dr_high8,
    output wire                    sr_high8,
    output wire                    alu_inputA_override,
    output wire                    alu_inputB_override,
    output wire                    alu_inputA_override_dup,
    ...
    bufferH16$ sr_dup(.out(HARDCODED_SR_DUP), .in(cs_HARDCODED_SR));
    ...
    `NOR_4(not_wierd_modrm_case_gate, 1, not_wierd_modrm_case, modrm_case_00100, modrm_case_00101, modrm_case_01100, modrm_case_10100)


    wire datasize_11, datasize_11_pre;
    wire datasize_00, datasize_00_pre;
    wire datasize_11_mod_11, datasize_11_mod_11_pre;
    wire datasize_00_mod_11, datasize_00_mod_11_pre;
    `AND_2(datasize_11_u, 1, datasize_11_pre, datasize[0], datasize[1])
    `NOR_2(datasize_00_u, 1, datasize_00_pre, datasize[0], datasize[1])
    `AND_3(datasize_11_mod_11_u, 1, datasize_11_mod_11_pre, datasize[0], datasize[1], mod_field_is_11)
    `NOR_3(datasize_00_mod_11_u, 1, datasize_00_mod_11_pre, datasize[0], datasize[1], mod_field_is_not_11)

    bufferH256$ dsize_11_buf (.out(datasize_11), .in(datasize_11_pre));
    bufferH256$ dsize_00_buf (.out(datasize_00), .in(datasize_00_pre));
    bufferH256$ dsize_11_mod_11_buf (.out(datasize_11_mod_11), .in(datasize_11_mod_11_pre));
    bufferH256$ dsize_00_mod_11_buf (.out(datasize_00_mod_11), .in(datasize_00_mod_11_pre));    
    ...
    wire rm_is_dr, rm_is_dr_pre;
    wire reg_is_dr, reg_is_dr_pre;
    wire reg_is_segment, reg_is_segment_pre;
    wire modrm_but_no_sr, modrm_but_no_sr_pre;
    wire not_rm_is_dr, not_rm_is_dr_pre;
    wire reg_is_seg_inv, reg_is_seg_inv_pre;

    `AND_2(rm_is_dr_gate, 1, rm_is_dr_pre, cs_MODRM_NEEDED, cs_RM_IS_DR)
    `AND_2(reg_is_dr_gate, 1, reg_is_dr_pre, cs_MODRM_NEEDED, cs_REG_IS_DR)
    `AND_2(reg_is_segment_gate, 1, reg_is_segment_pre, cs_MODRM_NEEDED, cs_REG_IS_SEGMENT)
    `INV_N(not_rm_is_dr_u, 1, rm_is_dr, not_rm_is_dr_pre)
    `INV_N(reg_seg_inv, 1, reg_is_segment, reg_is_seg_inv_pre)

    bufferH16$ rm_is_dr_buf         (.out(rm_is_dr),         .in(rm_is_dr_pre));
    bufferH16$ reg_is_dr_buf        (.out(reg_is_dr),        .in(reg_is_dr_pre));
    bufferH16$ reg_is_segment_buf   (.out(reg_is_segment),   .in(reg_is_segment_pre));
    bufferH16$ modrm_but_no_sr_buf  (.out(modrm_but_no_sr),  .in(modrm_but_no_sr_pre));
    bufferH16$ not_rm_is_dr_buf     (.out(not_rm_is_dr),     .in(not_rm_is_dr_pre));
    bufferH16$ reg_is_seg_inv_buf   (.out(reg_is_seg_inv),   .in(reg_is_seg_inv_pre));

    assign modrm_but_no_sr_pre = cs_MODRM_BUT_NO_SR;
    ...
    `AND_2(alu_inputA_over_gate_dup, 1, alu_inputA_override_dup, rm_is_dr, st_or_ld_op)
    `AND_2(alu_inputB_over_gate,     1, alu_inputB_override,     reg_is_dr, ld_op)
    `AND_2(alu_inputB_over_gate_dup, 1, alu_inputB_override_dup, reg_is_dr, ld_op_dup)
    ...
    wire rm_dr_not_wierd, rm_dr_not_wierd1, rm_dr_not_wierd2;
    wire reg_dr_not_segment;
    wire reg_dr_segment, reg_dr_segment1;

    `AND_2(rm_dr_not_wierd_gate, 1, rm_dr_not_wierd, rm_is_dr, not_wierd_modrm_case)
    `AND_2(rm_dr_not_wierd_gate1, 1, rm_dr_not_wierd1, rm_is_dr, not_wierd_modrm_case)
    `AND_2(rm_dr_not_wierd_gate2, 1, rm_dr_not_wierd2, rm_is_dr, not_wierd_modrm_case)
    ...
    `AND_2(reg_dr_segment_gate1, 1, reg_dr_segment1, reg_is_segment, reg_is_dr)
    

    wire sel_dr_a_or_b, sel_dr_a_or_b1;
    `OR_2(sel_dr_a_or_b_u, 1, sel_dr_a_or_b, rm_dr_not_wierd2, reg_dr_not_segment)
    `OR_2(sel_dr_a_or_b_u1, 1, sel_dr_a_or_b1, rm_dr_not_wierd2, reg_dr_not_segment)
    ...
    `AND_2(rm_dr_not_wierd_dr_wr_gate, 1, rm_dr_not_wierd_dr_wr, mod_field_is_11, cs_MODRM_NEEDED)
    ...
    `MUX_2(dr_rd_ab_u, 1, dr_rd_ab, 
            reg_dr_not_segment_dr_rd, rm_dr_not_wierd_dr_rd, rm_dr_not_wierd1)
    `MUX_4(dr_rd_cde_u, 1, dr_rd_cde, 
            dr_else_dr_rd, hardcoded_dr_dr_rd, reg_dr_segment_dr_rd, reg_dr_segment_dr_rd, 
    ...
    `MUX_2(dr_wr_ab_u, 1, dr_wr_ab, 
            reg_dr_not_segment_dr_wr, rm_dr_not_wierd_dr_wr, rm_dr_not_wierd1)
    `MUX_4(dr_wr_cde_u, 1, dr_wr_cde, 
            dr_else_dr_wr, hardcoded_dr_dr_wr, reg_dr_segment_dr_wr, reg_dr_segment_dr_wr, 
            {reg_dr_segment, cs_HARDCODED_DR})
    ...
    `MUX_2(dr_high8_ab_u, 1, dr_high8_ab, 
            reg_dr_not_segment_dr_high8, rm_dr_not_wierd_dr_high8, rm_dr_not_wierd1)
    `MUX_4(dr_high8_cde_u, 1, dr_high8_cde, 
            dr_else_dr_high8, hardcoded_dr_dr_high8, reg_dr_segment_dr_high8, reg_dr_segment_dr_high8, 
            {reg_dr_segment, cs_HARDCODED_DR})
    `MUX_2(dr_high8_u, 1, dr_high8, 
            dr_high8_cde, dr_high8_ab, sel_dr_a_or_b) 

    wire [`REG_ID_W-1:0] dr_id_ab;
    wire [`REG_ID_W-1:0] dr_id_cde;
    `MUX_2_H8(dr_id_ab_u, `REG_ID_W, dr_id_ab, 
            reg_dr_not_segment_dr_id, rm_dr_not_wierd_dr_id, rm_dr_not_wierd, rm_dr_not_wierd1)
    `MUX_4_H8(dr_id_cde_u, `REG_ID_W, dr_id_cde, 
            dr_else_dr_id, hardcoded_dr_dr_id, reg_dr_segment_dr_id, reg_dr_segment_dr_id, 
            {reg_dr_segment1, cs_HARDCODED_DR}, {reg_dr_segment, HARDCODED_DR_DUP})
    `MUX_2_H8(dr_id_u, `REG_ID_W, dr_id, 
            dr_id_cde, dr_id_ab, sel_dr_a_or_b1, sel_dr_a_or_b) 
    ...
    wire rm_dr_no_seg_reg_valid_sr, rm_dr_no_seg_reg_valid_sr1, rm_dr_no_seg_reg_valid_sr2;
    wire reg_dr_not_wierd;
    wire reg_seg_rm_dr, reg_seg_rm_dr1;
    ...
    `NOR_4(rm_dr_no_seg_reg_valid_sr_u1, 1, rm_dr_no_seg_reg_valid_sr1, not_rm_is_dr, reg_is_segment, modrm_but_no_sr, cs_HARDCODED_SR)
    `NOR_4(rm_dr_no_seg_reg_valid_sr_u2, 1, rm_dr_no_seg_reg_valid_sr2, not_rm_is_dr, reg_is_segment, modrm_but_no_sr, cs_HARDCODED_SR)
    ...
    `AND_2(reg_seg_rm_dr_u1, 1, reg_seg_rm_dr1, reg_is_segment, rm_is_dr)

    wire sel_sr_a_or_b, sel_sr_a_or_b1;
    `OR_2(sel_sr_a_or_b_u, 1, sel_sr_a_or_b, rm_dr_no_seg_reg_valid_sr, reg_dr_not_wierd)
    `OR_2(sel_sr_a_or_b_u1, 1, sel_sr_a_or_b1, rm_dr_no_seg_reg_valid_sr, reg_dr_not_wierd)
    ...
    assign rm_dr_no_seg_reg_valid_sr_sr_wr = 1'b0;
    ...
    `MUX_4(sr_wr_cde_u, 1, sr_wr_cde, 
            ...
            {reg_seg_rm_dr, HARDCODED_SR_DUP})
    ...
    `MUX_2(sr_high8_ab_u, 1, sr_high8_ab, 
            reg_dr_not_wierd_sr_high8, rm_dr_no_seg_reg_valid_sr_sr_high8, rm_dr_no_seg_reg_valid_sr2)
    ...
    wire [`REG_ID_W-1:0] sr_id_ab;
    wire [`REG_ID_W-1:0] sr_id_cde;
    `MUX_2_H8(sr_id_ab_u, `REG_ID_W, sr_id_ab, 
            reg_dr_not_wierd_sr_id, rm_dr_no_seg_reg_valid_sr_sr_id, rm_dr_no_seg_reg_valid_sr1, rm_dr_no_seg_reg_valid_sr2)
    `MUX_4_H8(sr_id_cde_u, `REG_ID_W, sr_id_cde, 
            sr_else_sr_id, hardcoded_sr_sr_id, reg_seg_rm_dr_sr_id, reg_seg_rm_dr_sr_id, 
            {reg_seg_rm_dr1, HARDCODED_SR_DUP}, {reg_seg_rm_dr, HARDCODED_SR_DUP})
    `MUX_2_H8(sr_id_u, `REG_ID_W, sr_id, 
            sr_id_cde, sr_id_ab, sel_sr_a_or_b1, sel_sr_a_or_b)  
    ...
    wire mod_rm_not_11_mod_rm_needed;
    `AND_2(mod_rm_not_11_mod_rm_needed_u, 1, mod_rm_not_11_mod_rm_needed, mod_field_is_not_11, cs_MODRM_NEEDED)
    `OR_2(ld_op_unmasked_u, 1, ld_op_unmasked, mod_rm_not_11_mod_rm_needed, cs_HARDCODED_LD_OP)

    wire rm_dr_mod_rm_not_11;
    `AND_2(rm_dr_mod_rm_not_11_u, 1, rm_dr_mod_rm_not_11, mod_field_is_not_11, rm_is_dr)
    `OR_2(st_op_unmasked_u, 1, st_op_unmasked, rm_dr_mod_rm_not_11, cs_HARDCODED_ST_OP)

    wire not_ld_op_cancel;
    wire not_st_op_cancel;
    `INV_N(ld_op_cancel_inv, 1, cs_LD_OP_CANCEL, not_ld_op_cancel)
    `INV_N(st_op_cancel_inv, 1, cs_ST_OP_CANCEL, not_st_op_cancel)

    `AND_2(ld_op_u,     1, ld_op,     ld_op_unmasked, not_ld_op_cancel)
    `AND_2(ld_op_u_dup, 1, ld_op_dup, ld_op_unmasked, not_ld_op_cancel)
    `AND_2(st_op_u,     1, st_op,     st_op_unmasked, not_st_op_cancel)
    `AND_2(st_op_u_dup, 1, st_op_dup, st_op_unmasked, not_st_op_cancel)
