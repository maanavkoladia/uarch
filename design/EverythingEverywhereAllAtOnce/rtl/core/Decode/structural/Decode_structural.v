// Decode_structural.v
//
// Pure Verilog-2005 top of the decode stage. Same internal structure as
// Decode_structural.sv (kept on disk as the SV reference) but with all
// SV-only constructs removed:
//   * No `import` of any package.
//   * No struct or typedef ports -- idm_outputs_t, fetch_outputs_t,
//     rr_outputs_t, dc_outputs_t, mem_outputs_t, exe_outputs_t,
//     wb_outputs_t, rr_latches_t, decode_outputs_t are unrolled into
//     individual flat wires whose widths come from the field types.
//   * No unpacked-array ports -- idm_slot data byte arrays are passed
//     as packed buses.
//   * `bool`/`logic`/`uint*_t` -> `wire [W-1:0]`.
//   * No struct literals -- all '{...} replaced with individual assigns.
//
// Only the ports and field references are flattened; the body keeps the
// same gate/macro/cell topology as Decode_structural.sv.
...
module Decode (
    ...
    //   Only fields actually consumed by the body are exposed:
    //     - per-slot {valid, br_valid, br_eip, br_btb_target}
    //     - per-slot data (16 bytes, packed as [127:0])
    //   br_xcl and valid_slots are not consumed and are not exposed.
    // ====================================================================
    ...
    //   Only consumed fields exposed:
    //     - exp_pipe_clear, exp_mode_jk, int_mode_jk
    // ====================================================================
    ...
    //   rr_latches_general_t expanded for both normal_latches and
    //   rep_latches.
    // ====================================================================
    ...
    assign flush = exe_outs_br_res_out_flush;

    wire decode_forward;
    `AND_2 (decode_forward_u, 1, decode_forward, rr_latch_we_o, next_rr_valid)

    // ------------------------------------------------------------------
    // Pack the four idm_slot data buses into the 512-bit predecode queue.
    //   slot 0 -> [127:0], slot 1 -> [255:128],
    //   slot 2 -> [383:256], slot 3 -> [511:384]
    // ------------------------------------------------------------------
    wire [511:0] flattened_queue;
    assign flattened_queue[127:0]   = idm_outs_idm_slots_0_data;
    assign flattened_queue[255:128] = idm_outs_idm_slots_1_data;
    assign flattened_queue[383:256] = idm_outs_idm_slots_2_data;
    assign flattened_queue[511:384] = idm_outs_idm_slots_3_data;
    ...
    // -------------------------------------------------------
    // Slot-select muxes for idm_outs_idm_slots[EIP[5:4]].* fields
    // -------------------------------------------------------
    wire [31:0] idm_slot_sel_br_eip;
    wire        idm_slot_sel_valid;
    wire        idm_slot_sel_br_valid;
    wire [31:0] idm_slot_sel_br_btb_target;
    `MUX_4(u_idm_slot_sel_br_eip,        32, idm_slot_sel_br_eip,
            idm_outs_idm_slots_0_br_eip,        idm_outs_idm_slots_1_br_eip,
            idm_outs_idm_slots_2_br_eip,        idm_outs_idm_slots_3_br_eip,
            EIP[5:4])
    `MUX_4(u_idm_slot_sel_valid,         1,  idm_slot_sel_valid,
            idm_outs_idm_slots_0_valid,         idm_outs_idm_slots_1_valid,
            idm_outs_idm_slots_2_valid,         idm_outs_idm_slots_3_valid,
            EIP[5:4])
    `MUX_4(u_idm_slot_sel_br_valid,      1,  idm_slot_sel_br_valid,
            idm_outs_idm_slots_0_br_valid,      idm_outs_idm_slots_1_br_valid,
            idm_outs_idm_slots_2_br_valid,      idm_outs_idm_slots_3_br_valid,
            EIP[5:4])
    `MUX_4(u_idm_slot_sel_br_btb_target, 32, idm_slot_sel_br_btb_target,
    ...
    // br_info_for_latches (br_info_t) -- unrolled
    wire        br_info_for_latches_valid;
    wire [31:0] br_info_for_latches_br_eip;
    wire        br_info_for_latches_br_xcl;
    wire        br_info_for_latches_br_pred_taken;
    wire [31:0] br_info_for_latches_speculative_target;
    assign br_info_for_latches_valid              = branch_info_valid;
    assign br_info_for_latches_br_eip             = branch_info_br_eip;
    assign br_info_for_latches_br_xcl             = branch_info_br_xcl;
    assign br_info_for_latches_br_pred_taken      = branch_info_br_pred_taken;
    assign br_info_for_latches_speculative_target = branch_info_speculative_target;

    `CMP_N(br_eip_cmp, 32, br_eip_eq_EIP, idm_slot_sel_br_eip, EIP)
    `AND_3(predicted_taken_gate, 1, predicted_taken, idm_slot_sel_valid, idm_slot_sel_br_valid, br_eip_eq_EIP)
    `MUX_2(predicted_target_mux, 32, predicted_target, 32'b0, idm_slot_sel_br_btb_target, predicted_taken)
    `OR_3(branch_present_gate,    1, branch_present,   temp_exe_cs_br_ucond, temp_exe_cs_relative_branch, temp_exe_cs_special_br)
    ...
    // rep_latch_holder (rr_latches_general_t) -- unrolled
    wire        rep_latch_holder_valid;
    wire        rep_latch_holder_cs_ST_SEL;
    wire        rep_latch_holder_cs_MODRM_NEEDED;
    wire        rep_latch_holder_cs_RM_IS_DR;
    wire        rep_latch_holder_cs_SWITCH_LD_ADDY;
    wire        rep_latch_holder_cs_LD_OP;
    wire        rep_latch_holder_cs_ST_OP;
    wire [`REG_ID_W-1:0] rep_latch_holder_cs_dr_id;
    wire [`REG_ID_W-1:0] rep_latch_holder_cs_sr_id;
    wire        rep_latch_holder_cs_dr_rd;
    wire        rep_latch_holder_cs_sr_rd;
    wire        rep_latch_holder_cs_eax_rd;
    wire        rep_latch_holder_cs_dr_wr;
    wire        rep_latch_holder_cs_sr_wr;
    wire        rep_latch_holder_cs_eax_wr;
    wire        rep_latch_holder_cs_MOVS_OP;
    wire [1:0]  rep_latch_holder_cs_datasize;
    wire        rep_latch_holder_cs_will_mod_zf;
    wire        rep_latch_holder_cs_seg_1_valid;
    wire [`REG_ID_W-1:0] rep_latch_holder_cs_seg_0_id;
    wire [`REG_ID_W-1:0] rep_latch_holder_cs_seg_1_id;
    wire        rep_latch_holder_cs_special_modrm_bs;
    wire        rep_latch_holder_cs_special_br;
    wire        rep_latch_holder_dc_cs_LD_OP;
    wire        rep_latch_holder_dc_cs_ST_OP;
    wire        rep_latch_holder_dc_cs_dr_upper8;
    wire        rep_latch_holder_dc_cs_sr_upper8;
    wire [1:0]  rep_latch_holder_dc_cs_datasize;
    wire        rep_latch_holder_mem_cs_ST_OP;
    wire        rep_latch_holder_mem_cs_LD_OP;
    wire        rep_latch_holder_exe_cs_ST_OP;
    wire [`EXE_OP_W-1:0]  rep_latch_holder_exe_cs_OP_TYPE;
    wire [`SRC_SEL_W-1:0] rep_latch_holder_exe_cs_alu_inputA_sel;
    wire [`SRC_SEL_W-1:0] rep_latch_holder_exe_cs_alu_inputB_sel;
    wire [`SRC_SEL_W-1:0] rep_latch_holder_exe_cs_branch_target_sel;
    wire        rep_latch_holder_exe_cs_shift_by_one;
    wire        rep_latch_holder_exe_cs_br_ucond;
    wire        rep_latch_holder_exe_cs_relative_branch;
    wire        rep_latch_holder_exe_cs_special_br;
    wire        rep_latch_holder_exe_cs_is_far;
    wire        rep_latch_holder_exe_cs_is_call;
    wire        rep_latch_holder_exe_cs_second_flag_needed;
    wire        rep_latch_holder_exe_cs_rep_no_zf_update;
    wire        rep_latch_holder_wb_cs_ST_OP;
    wire        rep_latch_holder_wb_cs_WB_DR;
    wire        rep_latch_holder_wb_cs_WB_SR;
    wire        rep_latch_holder_wb_cs_WB_EAX;
    wire        rep_latch_holder_br_info_valid;
    wire [31:0] rep_latch_holder_br_info_br_eip;
    wire        rep_latch_holder_br_info_br_xcl;
    wire        rep_latch_holder_br_info_br_pred_taken;
    wire [31:0] rep_latch_holder_br_info_speculative_target;
    wire [31:0] rep_latch_holder_NEIP;
    wire [31:0] rep_latch_holder_EIP;
    wire [31:0] rep_latch_holder_EAX;
    wire [63:0] rep_latch_holder_imm64;
    wire [`REG_ID_W-1:0] rep_latch_holder_sib_idx_id;
    wire [`REG_ID_W-1:0] rep_latch_holder_sib_base_id;
    wire        rep_latch_holder_sib_needed;
    wire [7:0]  rep_latch_holder_sib_scale;
    wire        rep_latch_holder_disp_needed;
    wire        rep_latch_holder_disp_size;
    wire [31:0] rep_latch_holder_displacement;

    wire clear_rep;

    wire external_set_zf;
    `AND_2(external_set_zf_gate, 1, external_set_zf, temp_rr_cs_will_mod_zf, decode_forward)
    ...
    `AND_2(u_cond2, 1, cond2, exe_outs_br_res_out_valid, flush)
    ...
    `MUX_4(u_eip_next, 32, eip_next,
            NEIP, idm_slot_sel_br_btb_target,
            exe_outs_br_res_out_br_target, 32'b0, penc_out[1:0])
    ...
    `NAND_2(u_exc_eip_sel, 1, exc_eip_sel, fetch_outs_exp_mode_jk[1], int_mode_jk_n)
    ...
    `NOR_3(u_valid_block,    1, valid_block,    going_to_halt, going_to_rep, fetch_outs_exp_pipe_clear)
    `AND_2(u_rr_latch_valid, 1, rr_latch_valid, next_rr_valid, valid_block)

    // latch EIP
    wire latch_eip_sel;
    wire [31:0] latch_eip;
    `OR_2(u_latch_eip_sel, 1, latch_eip_sel, fetch_outs_exp_mode_jk[0], fetch_outs_int_mode_jk)
    ...
    // ---------- normal_latches (rr_latches_general_t) field assigns ----------
    ...
    assign rr_latches_next_normal_latches_br_info_valid             = br_info_for_latches_valid;
    assign rr_latches_next_normal_latches_br_info_br_eip            = br_info_for_latches_br_eip;
    assign rr_latches_next_normal_latches_br_info_br_xcl            = br_info_for_latches_br_xcl;
    assign rr_latches_next_normal_latches_br_info_br_pred_taken     = br_info_for_latches_br_pred_taken;
    assign rr_latches_next_normal_latches_br_info_speculative_target= br_info_for_latches_speculative_target;
    ...
    // ---------- rep_latches (rr_latches_general_t) -- pass-through from holder ----------
    assign rr_latches_next_rep_latches_valid                        = rep_latch_holder_valid;

    assign rr_latches_next_rep_latches_cs_ST_SEL                    = rep_latch_holder_cs_ST_SEL;
    assign rr_latches_next_rep_latches_cs_MODRM_NEEDED              = rep_latch_holder_cs_MODRM_NEEDED;
    assign rr_latches_next_rep_latches_cs_RM_IS_DR                  = rep_latch_holder_cs_RM_IS_DR;
    assign rr_latches_next_rep_latches_cs_SWITCH_LD_ADDY            = rep_latch_holder_cs_SWITCH_LD_ADDY;
    assign rr_latches_next_rep_latches_cs_LD_OP                     = rep_latch_holder_cs_LD_OP;
    assign rr_latches_next_rep_latches_cs_ST_OP                     = rep_latch_holder_cs_ST_OP;
    assign rr_latches_next_rep_latches_cs_dr_id                     = rep_latch_holder_cs_dr_id;
    assign rr_latches_next_rep_latches_cs_sr_id                     = rep_latch_holder_cs_sr_id;
    assign rr_latches_next_rep_latches_cs_dr_rd                     = rep_latch_holder_cs_dr_rd;
    assign rr_latches_next_rep_latches_cs_sr_rd                     = rep_latch_holder_cs_sr_rd;
    assign rr_latches_next_rep_latches_cs_eax_rd                    = rep_latch_holder_cs_eax_rd;
    assign rr_latches_next_rep_latches_cs_dr_wr                     = rep_latch_holder_cs_dr_wr;
    assign rr_latches_next_rep_latches_cs_sr_wr                     = rep_latch_holder_cs_sr_wr;
    assign rr_latches_next_rep_latches_cs_eax_wr                    = rep_latch_holder_cs_eax_wr;
    assign rr_latches_next_rep_latches_cs_MOVS_OP                   = rep_latch_holder_cs_MOVS_OP;
    assign rr_latches_next_rep_latches_cs_datasize                  = rep_latch_holder_cs_datasize;
    assign rr_latches_next_rep_latches_cs_will_mod_zf               = rep_latch_holder_cs_will_mod_zf;
    assign rr_latches_next_rep_latches_cs_seg_1_valid               = rep_latch_holder_cs_seg_1_valid;
    assign rr_latches_next_rep_latches_cs_seg_0_id                  = rep_latch_holder_cs_seg_0_id;
    assign rr_latches_next_rep_latches_cs_seg_1_id                  = rep_latch_holder_cs_seg_1_id;
    assign rr_latches_next_rep_latches_cs_special_modrm_bs          = rep_latch_holder_cs_special_modrm_bs;
    assign rr_latches_next_rep_latches_cs_special_br                = rep_latch_holder_cs_special_br;

    assign rr_latches_next_rep_latches_dc_cs_LD_OP                  = rep_latch_holder_dc_cs_LD_OP;
    assign rr_latches_next_rep_latches_dc_cs_ST_OP                  = rep_latch_holder_dc_cs_ST_OP;
    assign rr_latches_next_rep_latches_dc_cs_dr_upper8              = rep_latch_holder_dc_cs_dr_upper8;
    assign rr_latches_next_rep_latches_dc_cs_sr_upper8              = rep_latch_holder_dc_cs_sr_upper8;
    assign rr_latches_next_rep_latches_dc_cs_datasize               = rep_latch_holder_dc_cs_datasize;

    assign rr_latches_next_rep_latches_mem_cs_ST_OP                 = rep_latch_holder_mem_cs_ST_OP;
    assign rr_latches_next_rep_latches_mem_cs_LD_OP                 = rep_latch_holder_mem_cs_LD_OP;

    assign rr_latches_next_rep_latches_exe_cs_ST_OP                 = rep_latch_holder_exe_cs_ST_OP;
    assign rr_latches_next_rep_latches_exe_cs_OP_TYPE               = rep_latch_holder_exe_cs_OP_TYPE;
    assign rr_latches_next_rep_latches_exe_cs_alu_inputA_sel        = rep_latch_holder_exe_cs_alu_inputA_sel;
    assign rr_latches_next_rep_latches_exe_cs_alu_inputB_sel        = rep_latch_holder_exe_cs_alu_inputB_sel;
    assign rr_latches_next_rep_latches_exe_cs_branch_target_sel     = rep_latch_holder_exe_cs_branch_target_sel;
    assign rr_latches_next_rep_latches_exe_cs_shift_by_one          = rep_latch_holder_exe_cs_shift_by_one;
    assign rr_latches_next_rep_latches_exe_cs_br_ucond              = rep_latch_holder_exe_cs_br_ucond;
    assign rr_latches_next_rep_latches_exe_cs_relative_branch       = rep_latch_holder_exe_cs_relative_branch;
    assign rr_latches_next_rep_latches_exe_cs_special_br            = rep_latch_holder_exe_cs_special_br;
    assign rr_latches_next_rep_latches_exe_cs_is_far                = rep_latch_holder_exe_cs_is_far;
    assign rr_latches_next_rep_latches_exe_cs_is_call               = rep_latch_holder_exe_cs_is_call;
    assign rr_latches_next_rep_latches_exe_cs_second_flag_needed    = rep_latch_holder_exe_cs_second_flag_needed;
    assign rr_latches_next_rep_latches_exe_cs_rep_no_zf_update      = rep_latch_holder_exe_cs_rep_no_zf_update;

    assign rr_latches_next_rep_latches_wb_cs_ST_OP                  = rep_latch_holder_wb_cs_ST_OP;
    assign rr_latches_next_rep_latches_wb_cs_WB_DR                  = rep_latch_holder_wb_cs_WB_DR;
    assign rr_latches_next_rep_latches_wb_cs_WB_SR                  = rep_latch_holder_wb_cs_WB_SR;
    assign rr_latches_next_rep_latches_wb_cs_WB_EAX                 = rep_latch_holder_wb_cs_WB_EAX;

    assign rr_latches_next_rep_latches_br_info_valid                = rep_latch_holder_br_info_valid;
    assign rr_latches_next_rep_latches_br_info_br_eip               = rep_latch_holder_br_info_br_eip;
    assign rr_latches_next_rep_latches_br_info_br_xcl               = rep_latch_holder_br_info_br_xcl;
    assign rr_latches_next_rep_latches_br_info_br_pred_taken        = rep_latch_holder_br_info_br_pred_taken;
    assign rr_latches_next_rep_latches_br_info_speculative_target   = rep_latch_holder_br_info_speculative_target;

    assign rr_latches_next_rep_latches_NEIP                         = rep_latch_holder_NEIP;
    assign rr_latches_next_rep_latches_EIP                          = rep_latch_holder_EIP;
    assign rr_latches_next_rep_latches_EAX                          = rep_latch_holder_EAX;
    assign rr_latches_next_rep_latches_imm64                        = rep_latch_holder_imm64;
    assign rr_latches_next_rep_latches_sib_idx_id                   = rep_latch_holder_sib_idx_id;
    assign rr_latches_next_rep_latches_sib_base_id                  = rep_latch_holder_sib_base_id;
    assign rr_latches_next_rep_latches_sib_needed                   = rep_latch_holder_sib_needed;
    assign rr_latches_next_rep_latches_sib_scale                    = rep_latch_holder_sib_scale;
    assign rr_latches_next_rep_latches_disp_needed                  = rep_latch_holder_disp_needed;
    assign rr_latches_next_rep_latches_disp_size                    = rep_latch_holder_disp_size;
    assign rr_latches_next_rep_latches_displacement                 = rep_latch_holder_displacement;
    ...
    rep_controller piece_of_shit_rep_controller (
        ...
        .rep_latches_valid                      (rep_latch_holder_valid),
        .rep_latches_cs_ST_SEL                  (rep_latch_holder_cs_ST_SEL),
        .rep_latches_cs_MODRM_NEEDED            (rep_latch_holder_cs_MODRM_NEEDED),
        .rep_latches_cs_RM_IS_DR                (rep_latch_holder_cs_RM_IS_DR),
        .rep_latches_cs_SWITCH_LD_ADDY          (rep_latch_holder_cs_SWITCH_LD_ADDY),
        .rep_latches_cs_LD_OP                   (rep_latch_holder_cs_LD_OP),
        .rep_latches_cs_ST_OP                   (rep_latch_holder_cs_ST_OP),
        .rep_latches_cs_dr_id                   (rep_latch_holder_cs_dr_id),
        .rep_latches_cs_sr_id                   (rep_latch_holder_cs_sr_id),
        .rep_latches_cs_dr_rd                   (rep_latch_holder_cs_dr_rd),
        .rep_latches_cs_sr_rd                   (rep_latch_holder_cs_sr_rd),
        .rep_latches_cs_eax_rd                  (rep_latch_holder_cs_eax_rd),
        .rep_latches_cs_dr_wr                   (rep_latch_holder_cs_dr_wr),
        .rep_latches_cs_sr_wr                   (rep_latch_holder_cs_sr_wr),
        .rep_latches_cs_eax_wr                  (rep_latch_holder_cs_eax_wr),
        .rep_latches_cs_MOVS_OP                 (rep_latch_holder_cs_MOVS_OP),
        .rep_latches_cs_datasize                (rep_latch_holder_cs_datasize),
        .rep_latches_cs_will_mod_zf             (rep_latch_holder_cs_will_mod_zf),
        .rep_latches_cs_seg_1_valid             (rep_latch_holder_cs_seg_1_valid),
        .rep_latches_cs_seg_0_id                (rep_latch_holder_cs_seg_0_id),
        .rep_latches_cs_seg_1_id                (rep_latch_holder_cs_seg_1_id),
        .rep_latches_cs_special_modrm_bs        (rep_latch_holder_cs_special_modrm_bs),
        .rep_latches_cs_special_br              (rep_latch_holder_cs_special_br),
        .rep_latches_dc_cs_LD_OP                (rep_latch_holder_dc_cs_LD_OP),
        .rep_latches_dc_cs_ST_OP                (rep_latch_holder_dc_cs_ST_OP),
        .rep_latches_dc_cs_dr_upper8            (rep_latch_holder_dc_cs_dr_upper8),
        .rep_latches_dc_cs_sr_upper8            (rep_latch_holder_dc_cs_sr_upper8),
        .rep_latches_dc_cs_datasize             (rep_latch_holder_dc_cs_datasize),
        .rep_latches_mem_cs_ST_OP               (rep_latch_holder_mem_cs_ST_OP),
        .rep_latches_mem_cs_LD_OP               (rep_latch_holder_mem_cs_LD_OP),
        .rep_latches_exe_cs_ST_OP               (rep_latch_holder_exe_cs_ST_OP),
        .rep_latches_exe_cs_OP_TYPE             (rep_latch_holder_exe_cs_OP_TYPE),
        .rep_latches_exe_cs_alu_inputA_sel      (rep_latch_holder_exe_cs_alu_inputA_sel),
        .rep_latches_exe_cs_alu_inputB_sel      (rep_latch_holder_exe_cs_alu_inputB_sel),
        .rep_latches_exe_cs_branch_target_sel   (rep_latch_holder_exe_cs_branch_target_sel),
        .rep_latches_exe_cs_shift_by_one        (rep_latch_holder_exe_cs_shift_by_one),
        .rep_latches_exe_cs_br_ucond            (rep_latch_holder_exe_cs_br_ucond),
        .rep_latches_exe_cs_relative_branch     (rep_latch_holder_exe_cs_relative_branch),
        .rep_latches_exe_cs_special_br          (rep_latch_holder_exe_cs_special_br),
        .rep_latches_exe_cs_is_far              (rep_latch_holder_exe_cs_is_far),
        .rep_latches_exe_cs_is_call             (rep_latch_holder_exe_cs_is_call),
        .rep_latches_exe_cs_second_flag_needed  (rep_latch_holder_exe_cs_second_flag_needed),
        .rep_latches_exe_cs_rep_no_zf_update    (rep_latch_holder_exe_cs_rep_no_zf_update),
        .rep_latches_wb_cs_ST_OP                (rep_latch_holder_wb_cs_ST_OP),
        .rep_latches_wb_cs_WB_DR                (rep_latch_holder_wb_cs_WB_DR),
        .rep_latches_wb_cs_WB_SR                (rep_latch_holder_wb_cs_WB_SR),
        .rep_latches_wb_cs_WB_EAX               (rep_latch_holder_wb_cs_WB_EAX),
        .rep_latches_br_info_valid              (rep_latch_holder_br_info_valid),
        .rep_latches_br_info_br_eip             (rep_latch_holder_br_info_br_eip),
        .rep_latches_br_info_br_xcl             (rep_latch_holder_br_info_br_xcl),
        .rep_latches_br_info_br_pred_taken      (rep_latch_holder_br_info_br_pred_taken),
        .rep_latches_br_info_speculative_target (rep_latch_holder_br_info_speculative_target),
        .rep_latches_NEIP                       (rep_latch_holder_NEIP),
        .rep_latches_EIP                        (rep_latch_holder_EIP),
        .rep_latches_EAX                        (rep_latch_holder_EAX),
        .rep_latches_imm64                      (rep_latch_holder_imm64),
        .rep_latches_sib_idx_id                 (rep_latch_holder_sib_idx_id),
        .rep_latches_sib_base_id                (rep_latch_holder_sib_base_id),
        .rep_latches_sib_needed                 (rep_latch_holder_sib_needed),
        .rep_latches_sib_scale                  (rep_latch_holder_sib_scale),
        .rep_latches_disp_needed                (rep_latch_holder_disp_needed),
        .rep_latches_disp_size                  (rep_latch_holder_disp_size),
        .rep_latches_displacement               (rep_latch_holder_displacement),
        .clear_rep                              (clear_rep)
    );
