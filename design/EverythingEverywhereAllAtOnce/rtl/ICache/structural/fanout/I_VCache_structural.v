module I_VCache (
    ...
    // hit_idx_0/1 each had fanout 157 (drives selects of 28-bit and 128-bit
    // 4-way muxes plus update/decode). Re-buffer with bufferH256$ from lib2.
    wire hit_idx_0_pre, hit_idx_1_pre;

    `OR_4(u_currTagHit, 1, currTagHit, match_0, match_1, match_2, match_3)
    `OR_2(u_hit_idx_0, 1, hit_idx_0_pre, match_1, match_3)
    `OR_2(u_hit_idx_1, 1, hit_idx_1_pre, match_2, match_3)
    bufferH256$ u_hit_idx_0_buf (.out(hit_idx_0), .in(hit_idx_0_pre));
    bufferH256$ u_hit_idx_1_buf (.out(hit_idx_1), .in(hit_idx_1_pre));
    ...
    // u_hit had fanout 134; re-buffer with bufferH256$ from lib2.
    wire hit_pre;
    `AND_3(u_hit,            1, hit_pre, busy_bar, en_i, currTagHit)
