module ICache (
    ...
    // curr_v_addr_to_use is triplicated -- one 32-bit copy per submodule
    // destination (TagStore/DataStore/I_VCache). Each u_vaddr_mux_x covers
    // only that submodule's loads, so each per-bit fanout is roughly 1/3
    // of the original.
    wire [31:0]  curr_v_addr_a;  // -> ICache_TagStore
    wire [31:0]  curr_v_addr_b;  // -> ICache_DataStore
    wire [31:0]  curr_v_addr_c;  // -> I_VCache
    wire [31:0]  curr_v_addr_to_use;  // legacy alias = curr_v_addr_a
    assign curr_v_addr_to_use = curr_v_addr_a;
    ...
    // Triplicated x3 -- the 32 mux2$ violations on u_vaddr_mux were caused
    // by one 32-bit mux feeding three submodules + local logic. Three
    // separate 32-bit muxes each drive one submodule, distributing the
    // per-bit fanout three ways.
    // ------------------------------------------------------------------
    `MUX_2(u_vaddr_mux_a, 32, curr_v_addr_a, v_addr_i, saved_vAddr, busy)
    `MUX_2(u_vaddr_mux_b, 32, curr_v_addr_b, v_addr_i, saved_vAddr, busy)
    `MUX_2(u_vaddr_mux_c, 32, curr_v_addr_c, v_addr_i, saved_vAddr, busy)
    ...
    // ------------------------------------------------------------------
    // u_set_case (fanout=5 in original) -- replicate the AND_2 x 2.
    // Distribute the 4-5 internal uses across set_case_a (driving the
    // small fanout latch/OR loads) and set_case_b (driving the wide WE
    // ports of the 32-bit and 128-bit registers).
    wire set_case_a, set_case_b;
    `AND_2(u_set_case_a, 1, set_case_a, LD_IC_SWAP_BUF, icache_tag_V)
    `AND_2(u_set_case_b, 1, set_case_b, LD_IC_SWAP_BUF, icache_tag_V)

    wire we_swapbuf_v, clr_bar, d_swapbuf_v;
    `OR_2 (u_we_sb_v, 1, we_swapbuf_v, set_case_a, i_vcache_swapBuf_V_Clr)
    `INV_N(u_clr_bar, 1, i_vcache_swapBuf_V_Clr, clr_bar)
    `AND_2(u_d_sb_v,  1, d_swapbuf_v, set_case_a, clr_bar)
    ...
    `REG_RST_WE(u_swapbuf_la,   32, clk, rst, set_case_b, swap_lineAddr_d, icache_swapbuf_lineAddr)
    `REG_RST_WE(u_swapbuf_line, 128, clk, rst, set_case_b, icache_dataLines, icache_swapbuf_line)
