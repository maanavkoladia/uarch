// Structural Verilog 2005 port of DCache_Bank_TagStore.
// Reference SV: rtl/DCache/DCache_Block/DCache_Bank/DCache_Bank_TagStore.sv
// All logic is via STDCell macros. The 6-bit tag word is held in one
// `ram8b8w$` cell (address = 3-bit index, data = {2'b0, tag}). Per-line
// valid and dirty bits are held in REG_RST_WE flops (8 of each).
// Phased clock for RAM write timing matches the SV `#CLK_PHASE_DELAY`
// trick using BUFFER_DELAY (10 stages * 0.25 ns = 2.5 ns).
...
module DCache_Bank_TagStore (
    ...
    // ---------------------------------------------------------------
    // Address slices (wire aliasing)
    // ---------------------------------------------------------------
    wire [2:0] index;       // p_addr[8:6]
    wire [5:0] tag_in;      // p_addr[14:9]
    assign index  = p_addr_i[8:6];
    assign tag_in = p_addr_i[14:9];

    // ---------------------------------------------------------------
    // Index decoder: 3-bit index -> 8 one-hot select lines
    // ---------------------------------------------------------------
    wire [7:0] index_decoded;
    `DECODER_N(u_index_dec, 3, index, index_decoded)

    // ---------------------------------------------------------------
    // Per-line valid / dirty flop banks
    // ---------------------------------------------------------------
    wire [7:0] valid_q;
    wire [7:0] dirty_q;
    ...
    // Per-line write-enable / data nets
    wire [7:0] valid_we;
    wire [7:0] dirty_we;
    wire [7:0] dirty_d;
    ...
    // Generate per-line gating + flops
    genvar gtl;
    generate
        for (gtl = 0; gtl < 8; gtl = gtl + 1) begin : g_tag_meta
            // valid: we = decoded_index & (fill3 | ld_From_V_Swap), d = 1
            ...
            // dirty:
            //   ws_event   = decoded_index & writeSuccess              (priority 1)
            //   fill_event = decoded_index & (fill3 | ld_From_V_Swap)  (priority 2)
            //   we = ws_event | fill_event
            //   d  = ws_event ? 1 : (ld_From_V_Swap & V_Cache_SwapBuf_DirtyBit)
    ...
    // ---------------------------------------------------------------
    // Read-side MUX_8 selecting current line's valid / dirty
    // ---------------------------------------------------------------
    ...
    // ---------------------------------------------------------------
    // RAM write-enable phased gate
    //   WR_actual = ~(rst & (fill3 | ld_From_V_Swap) & clk_45_phase)
    ...
    // ---------------------------------------------------------------
    // RAM output-enable gate (active-low)
    //   read_event = ((oe | we) & ~busy) | write2_Dwap
    //   OE_actual = ~(rst & read_event)
    // ---------------------------------------------------------------
    wire oe_or_we;
    wire not_busy;
    wire access_active;
    ...
    // ---------------------------------------------------------------
    // RAM cell — 8 words x 8 bits. DIN packs the 6-bit tag in [5:0],
    // upper 2 bits zero. tagOut_o picks the low 6 bits of DOUT.
    // ---------------------------------------------------------------
