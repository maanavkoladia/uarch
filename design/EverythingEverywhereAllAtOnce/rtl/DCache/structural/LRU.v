...
// LRU_q[0] = ROOT bit, LRU_q[1] = LEFT_LEAF, LRU_q[2] = RIGHT_LEAF.
// Each tree-bit points TOWARD the MRU subtree, so the LRU way is reached
// by inverting the bits along the traversal.
...
module LRU (
    ...
    // ----- Next-state -------------------------------------------------------
    // newLRU[0] = savedIDX[1]                       (root points at accessed half)
    assign newLRU[0] = savedIDX[1];

    // newLRU[1] (LEFT_LEAF) updates only when savedIDX[1]=0 (left subtree
    // accessed); otherwise hold:
    //     newLRU[1] = savedIDX_1_inv ? savedIDX[0] : LRU_q[1]
    `MUX_2(mux_left_leaf,  1, newLRU[1], LRU_q[1], savedIDX[0], savedIDX_1_inv)

    // newLRU[2] (RIGHT_LEAF) updates only when savedIDX[1]=1 (right subtree
    // accessed); otherwise hold:
    //     newLRU[2] = savedIDX[1] ? savedIDX[0] : LRU_q[2]
    `MUX_2(mux_right_leaf, 1, newLRU[2], LRU_q[2], savedIDX[0], savedIDX[1])

    // ----- Storage ----------------------------------------------------------
    // Active-low rst (MPS_reg_rst_we$); WE = updateLRU.
