...
//
// Wires up the FSM, TagStore, DataStore, the saved-request flop, and the
// dcache_bank_swapBuf. Drives a flat 279-bit `d_cache_bank_outputs_t` bus.
...
module DCache_Bank (
    ...
    input  wire [`CACHE_LINES_SIZE_BITS     - 1 : 0]  dataBus,             // CL-wide (only low 32 used by datastore)
    output wire [`DCB_OUT_W                 - 1 : 0]  outputs_o
);

    //==================================================================
    // Field extraction from incoming packed buses
    //==================================================================
    // V_Cache outputs
    wire        vc_miss;
    wire        vc_dswap_vclr;
    ...
    assign breq_vec       = blockReq_i[`BREQ_VEC_UB:`BREQ_VEC_LB];
    assign breq_data      = blockReq_i[`BREQ_DATA_UB:`BREQ_DATA_LB];
    ...
    //==================================================================
    // dcache_bank_swapBuf (145-bit) flop bank
    //==================================================================
    wire        dswap_valid_q;
    wire        dswap_dirty_q;
    ...
    // Address composition (15 bits): {tag(6), idx(3), bank(2), 4'b0000}
    wire [`P_ADDR_W - 1 : 0] swap_lineAddr;
    assign swap_lineAddr = {riu_tag, riu_idx, riu_bank, 4'b0000};

    // Tag/Data submodule signals (forward decls)
    ...
    // dirty / addr / line: WE = write_to_dswap
    ...
    `INV_N(inv_blockbusy, 1, block_busy_inv, block_busy_i)
    ...
    `INV_N(inv_hit,       1, hit_inv,        hit)
    `AND_2(and_miss,      1, miss,           doAccess, hit_inv)
    `AND_2(and_writeSucc, 1, writeSuccess2TagStore, hit, riu_we)

    //==================================================================
    // Output bus assembly (d_cache_bank_outputs_t flat layout)
    //==================================================================
    //   dcache_swapBuf packed in SWAP layout:
    //     [0]=valid, [1]=dirty, [16:2]=addr, [144:17]=line
