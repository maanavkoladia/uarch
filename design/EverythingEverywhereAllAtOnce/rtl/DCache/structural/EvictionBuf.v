...
module EvictionBuf (
    ...
    `AND_2(and_commitp, 1, commit_pre,   load_action_inv, eb_clr_inv)
    `AND_2(and_commita, 1, commit_action,commit_pre,      set_commiting)
    `OR_2 (or_v_we,     1, valid_we,     load_action,     clear_action)
    `OR_3 (or_c_we,     1, commiting_we, load_action,     clear_action,    commit_action)
    ...
    // Hit detection: 11-bit eq split into 6+5 (CMP_N supports 2,3,4,5,6,8,9)
    wire validReq;
    wire cmp_lo;
    wire cmp_hi;
    wire cmp_eq;
