...
module tb_restOfStages ();
    ...
    task automatic print_fetch();
        ...
        $fdisplay(log_fd, "  SPC       = 0x%08h   next_spc = 0x%08h   spc+16 = 0x%08h",
                  uut_core.fetch_unit.SPC, uut_core.fetch_unit.next_spc,
                  uut_core.fetch_unit.spc_16);
        $fdisplay(log_fd, "  spc_sel   = %s   br_tgt_sel=%0b   flush_reg=%0b", get_spc_sel_name(
                  uut_core.fetch_unit.spc_sel_logic_outs.sel),
        ...
        $fdisplay(
            log_fd, "  TLB: v_addr=0x%08h  p_addr=0x%08h  valid=%0b  gp=%0b  pf=%0b  f_exp=%0b",
            uut_core.fetch_unit.seg_xlation_out, uut_core.fetch_unit.tlb_outs.physical_addr,
            uut_core.fetch_unit.tlb_outs.physical_addr_valid, uut_core.fetch_unit.tlb_outs.gp_exp,
            uut_core.fetch_unit.tlb_outs.pageFault, uut_core.fetch_unit.f_exp);
        $fdisplay(log_fd, "  BTB: hit=%0b  eip=0x%08h  tgt=0x%08h  XCL=%0b  ucond=%0b",
                  uut_core.fetch_unit.btb_outs.hit, uut_core.fetch_unit.btb_outs.br_eip,
                  uut_core.fetch_unit.btb_outs.br_target, uut_core.fetch_unit.btb_outs.XCL,
        ...
        $fdisplay(log_fd, "  ICache: hit=%0b  icache_en=%0b", icache_2_core.hit,
                  uut_core.fetch_outputs.fetch_2_icache.icache_en);
    endtask
    ...
    task automatic print_idm();
        ...
        for (int i = 0; i < NUM_IDM_SLOTS; i++)
            $fwrite(
                log_fd, " [%0d]=%0b", i, uut_core.fetch_unit.idm_invalidate_logic_outs.invalidate[i]
            );
    ...
    task automatic print_decode();
        ...
        $fdisplay(log_fd,
                  ...
                  uut_core.decode_outputs.valid, uut_core.decode_outputs.eip,
                  uut_core.decode_outputs.invalid_instruction, uut_core.decode_outputs.decode_gp,
    ...
    task automatic print_rr_latches();
        ...
        begin
            ...
            $fdisplay(log_fd,
                      "  dr=%s  sr=%s  dr_rd=%0b  sr_rd=%0b  dr_wr=%0b  sr_wr=%0b  datasize=%0d",
                      get_reg_name(L.cs.dr_id), get_reg_name(L.cs.sr_id), L.cs.dr_rd, L.cs.sr_rd,
                      L.cs.dr_wr, L.cs.sr_wr, L.cs.datasize);
            $fdisplay(log_fd, "  LD_OP=%0b  ST_OP=%0b  MODRM=%0b  RM_IS_DR=%0b", L.cs.LD_OP,
                      L.cs.ST_OP, L.cs.MODRM_NEEDED, L.cs.RM_IS_DR);
            $fdisplay(log_fd, "  EXE_CS: OP=%s  br_ucond=%0b  rel_br=%0b  special=%0b  far=%0b",
                      get_op_name(L.exe_cs.OP_TYPE), L.exe_cs.br_ucond, L.exe_cs.relative_branch,
                      L.exe_cs.special_br, L.exe_cs.is_far);
            $fdisplay(log_fd,
                      "  br_info: v=%0b  eip=0x%08h  xcl=%0b  pred_taken=%0b  spec_tgt=0x%08h",
                      L.br_info.valid, L.br_info.br_eip, L.br_info.br_xcl, L.br_info.br_pred_taken,
                      L.br_info.speculative_target);
            if (L.sib_needed)
                $fdisplay(
                    log_fd,
                    "  SIB: idx=%s  base=%s  scale=%0d",
                    get_reg_name(
                        L.sib_idx_id
                    ),
                    get_reg_name(
            ...
            if (L.disp_needed)
                $fdisplay(
                    log_fd, "  DISP: size=%s  val=0x%08h", L.disp_size ? "32" : "8", L.displacement
                );
            $fdisplay(log_fd, "  WB_CS: ST_OP=%0b  WB_DR=%0b  WB_SR=%0b", L.wb_cs.ST_OP,
                      L.wb_cs.WB_DR, L.wb_cs.WB_SR);
        end
    endtask
    ...
    task automatic print_rr_outputs();
        ...
        $fdisplay(log_fd, "  valid=%0b  stall=%0b ecx_sb=%0b  cs_sb=%0b  dc_we=%0b",
                  uut_core.rr_outputs.valid, uut_core.rr_outputs.stall, uut_core.rr_outputs.ecx_sb,
                  uut_core.rr_outputs.codeSeg_sb, uut_core.rr_outputs.dc_stage_latch_we);
    endtask
    ...
    task automatic print_dc_latches();
        ...
        begin
            ...
            $fdisplay(log_fd, "  valid=%0b  EIP=0x%08h  NEIP=0x%08h", L.valid, L.EIP, L.NEIP);
            $fdisplay(log_fd, "  dr=%s(0x%016h)  sr=%s(0x%016h)", get_reg_name(L.dr_id), L.dr_data,
                      get_reg_name(L.sr_id), L.sr_data);
            $fdisplay(log_fd, "  CS: LD=%0b  ST=%0b  upper8=%0b  dsize=%0d", L.cs.LD_OP,
                      L.cs.ST_OP, L.cs.upper8, L.cs.datasize);
            $fdisplay(log_fd, "  LD: vaddr=0x%08h  next_vaddr=0x%08h  limit=0x%08h", L.ld_vaddy,
                      L.next_ld_vaddy, L.seg0_limit_w_datasize);
            $fdisplay(log_fd, "  ST: vaddr=0x%08h  next_vaddr=0x%08h  limit=0x%08h", L.st_vaddy,
                      L.next_st_vaddy, L.seg1_limit_w_datasize);
            $fdisplay(log_fd, "  br_info: v=%0b  eip=0x%08h  pred_taken=%0b", L.br_info.valid,
                      L.br_info.br_eip, L.br_info.br_pred_taken);
            $fdisplay(log_fd, "  EXE_CS: OP=%s  WB_CS: ST=%0b DR=%0b SR=%0b", get_op_name(
                      L.exe_cs.OP_TYPE), L.wb_cs.ST_OP, L.wb_cs.WB_DR, L.wb_cs.WB_SR);
    ...
    task automatic print_dc_outputs();
        ...
        $fdisplay(log_fd, "  valid=%0b  stall=%0b  mem_we=%0b", uut_core.dc_outputs.valid,
                  uut_core.dc_outputs.stall, uut_core.dc_outputs.exp_present,
                  uut_core.dc_outputs.exp_pf, uut_core.dc_outputs.mem_stage_latch_we);
        $fdisplay(log_fd,
                  "  ld0: V=%0b addr=0x%04h   ld1: V=%0b addr=0x%04h   mio: V=%0b addr=0x%04h",
    ...
    task automatic print_mem_latches();
        ...
        begin
            ...
            $fdisplay(log_fd, "  dr=%s(0x%016h)  sr=%s(0x%016h)", get_reg_name(L.dr_id), L.dr_data,
                      get_reg_name(L.sr_id), L.sr_data);
            $fdisplay(log_fd, "  CS: LD=%0b  ST=%0b   data_size_vec=0x%01h", L.cs.LD_OP,
                      L.cs.ST_OP, L.data_size_vec);
            $fdisplay(log_fd, "  LD: xcl=%0b  paddr0=0x%04h  paddr1=0x%04h  swap=%0b", L.LD_XCL,
                      L.LD_PADDR_0, L.LD_PADDR_1, L.swapLines);
            $fdisplay(log_fd, "  ST: xcl=%0b  paddr0=0x%04h  paddr1=0x%04h  MIO=%0b", L.ST_XCL,
                      L.ST_PADDR_0, L.ST_PADDR_1, L.MIO);
            $fdisplay(log_fd, "  EXE_CS: OP=%s  WB_CS: ST=%0b DR=%0b SR=%0b", get_op_name(
                      L.exe_cs.OP_TYPE), L.wb_cs.ST_OP, L.wb_cs.WB_DR, L.wb_cs.WB_SR);
            $fdisplay(log_fd, "  br_info: v=%0b  eip=0x%08h  pred_taken=%0b", L.br_info.valid,
                      L.br_info.br_eip, L.br_info.br_pred_taken);
    ...
    task automatic print_mem_outputs();
        ...
        $fdisplay(log_fd, "  valid=%0b  stall=%0b  exe_we=%0b  ST_OP=%0b  ST_XCL=%0b",
                  ...
                  uut_core.mem_outputs.exe_stage_latch_we, uut_core.mem_outputs.ST_OP,
                  uut_core.mem_outputs.ST_XCL);
    ...
    task automatic print_exe_latches();
        ...
        begin
            ...
            $fdisplay(log_fd, "  dr=%s(0x%016h)  sr=%s(0x%016h)", get_reg_name(L.dr_id), L.dr_data,
                      get_reg_name(L.sr_id), L.sr_data);
            $fdisplay(log_fd, "  CS: OP=%s  ST=%0b   data_size_vec=0x%01h", get_op_name(
                      L.cs.OP_TYPE), L.cs.ST_OP, L.data_size_vec);
            ...
            $fdisplay(log_fd, "  WB_CS: ST=%0b DR=%0b SR=%0b", L.wb_cs.ST_OP, L.wb_cs.WB_DR,
                      L.wb_cs.WB_SR);
    ...
    task automatic print_wb_latches();
        ...
        begin
            ...
            $fdisplay(log_fd, "  valid=%0b  CS: ST=%0b  WB_DR=%0b  WB_SR=%0b", L.valid, L.cs.ST_OP,
                      L.cs.WB_DR, L.cs.WB_SR);
            $fdisplay(log_fd, "  dr=%s(0x%016h)  sr=%s(0x%016h)", get_reg_name(L.dr_id), L.dr_data,
                      get_reg_name(L.sr_id), L.sr_data);
            $fdisplay(
                log_fd,
                "  ST: xcl=%0b  paddr0=0x%04h  bv0=0x%04h  paddr1=0x%04h  bv1=0x%04h  MIO=%0b",
    ...
    task automatic print_wb_outputs();
        ...
        $fdisplay(log_fd, "  valid=%0b  wb_stall=%0b", uut_core.wb_outputs.valid,
                  uut_core.wb_outputs.wb_stall);
        $fdisplay(log_fd, "  DR0: we=%0b  id=%s  data=0x%016h", uut_core.wb_outputs.DR_0_we,
                  get_reg_name(uut_core.wb_outputs.DR_0_id), uut_core.wb_outputs.DR_0_data);
        $fdisplay(log_fd, "  DR1: we=%0b  id=%s  data=0x%016h", uut_core.wb_outputs.DR_1_we,
                  get_reg_name(uut_core.wb_outputs.DR_1_id), uut_core.wb_outputs.DR_1_data);
    endtask
    ...
    task automatic print_stalls();
        ...
        $fdisplay(log_fd, "  RR_stall=%0b  DC_stall=%0b  MEM_stall=%0b  WB_stall=%0b",
                  uut_core.rr_outputs.stall, uut_core.dc_outputs.stall, uut_core.mem_outputs.stall,
                  uut_core.wb_outputs.wb_stall);
        $fdisplay(log_fd, "  flush=%0b  farFlush=%0b  exp_pipe_clear=%0b  int_pipe_clear=%0b",
                  uut_core.exe_outputs.br_res_out.flush, uut_core.exe_outputs.br_res_out.farFlush,
    ...
    task automatic print_regfile();
        ...
        for (int i = 0; i < NUM_REGS; i++) begin
            $fwrite(log_fd, "%s=0x%08h  ", get_reg_name(reg_ids_e'(i)),
    ...
    task automatic print_scoreboard();
        ...
        $fdisplay(log_fd, "  dep_stall=%0b  ecx_sb=%0b  cs_sb=%0b",
                  uut_core.rr_unit.reg_sb_unit.dep_stall, uut_core.rr_unit.reg_sb_unit.ecx_sb,
                  uut_core.rr_unit.reg_sb_unit.codeSeg_sb);
    endtask

    // --- STORE QUEUES ---
    task automatic print_one_stq(int idx, st_q_outputs_t outs);
        $fdisplay(log_fd, "  STQ[%0d]: full=%0b  empty=%0b  push_fail=%0b", idx, outs.full,
                  outs.empty, outs.push_fail);
        if (!outs.empty) begin
            $fdisplay(log_fd, "    HEAD: addr=0x%04h  bv=0x%04h", outs.head_address, outs.bit_vec);
        end
        for (int e = 0; e < ST_Q_DEPTH; e++) begin
            if (outs.valid[e]) $fdisplay(log_fd, "    [%0d] V=1 addr=0x%04h", e, outs.address[e]);
        end
    endtask
    ...
    task automatic print_mio_queue();
        ...
        $fdisplay(log_fd, "  full=%0b  empty=%0b", uut_core.write_back_unit.mio_q_inst.full,
    ...
    task automatic print_dcache_arb();
        ...
        for (int i = 0; i < DCACHE_NUM_BLOCKS; i++) begin
            $fdisplay(log_fd,
                      "  blk[%0d]: oe=%0b  we=%0b  addr=0x%04h  hit=%0b  ws=%0b  sch_req=%0d", i,
                      uut_dcache.req_2_blocks[i].oe, uut_dcache.req_2_blocks[i].we,
                      uut_dcache.req_2_blocks[i].p_addr, uut_dcache.hitVec[i],
                      dcache_2_core.writeSuccess[i], uut_dcache.out2Sch_o.req[i]);
        end
    endtask
    ...
    initial begin
        ...
        DelayClks(20);
        @(posedge clk) @(posedge clk) force uut_core.fetch_unit.SPC = 32'h1000;
