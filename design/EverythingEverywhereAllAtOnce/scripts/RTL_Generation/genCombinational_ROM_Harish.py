...
* Output wiring : ROMs sharing the same slice index share one tri-state wire (dout_s{sl}).
                  Only one bank's OE is high at a time, so all others are Z — safe wired bus.
                  A single assign per output port connects from the shared slice wire.
...
def dout_shared_wire(sl):       return f"dout_s{sl}"            # shared across all banks for slice sl
...
def emit_internal_wires(out, n_inputs, num_addr_banks, num_out_slices, upper_addr_bits):
    ...
    # One shared 64-bit wire per output slice; all banks in the same slice connect to it.
    for sl in range(num_out_slices):
        out.append(f"    wire [63:0] {dout_shared_wire(sl)};")
    out.append("")
...
def emit_rom_instantiations(out, num_addr_banks, num_out_slices):
    ...
    out.append("    // ROMs in the same slice share one tri-state output wire across banks.")
    out.append("    // ROMs in different slices of the same bank use separate wires.")
    for b in range(num_addr_banks):
        for sl in range(num_out_slices):
            name = rom_name(b, sl)
            oe   = oe_wire(b)
            dout = dout_shared_wire(sl)   # shared wire for this slice across all banks
...
def emit_output_assigns(out, output_cols, num_addr_banks, num_out_slices, n_outputs):
    """
    Single assign per output port from the shared per-slice wire.

    dout_s{sl} is driven by all bank ROMs for that slice wired together.
    Only one bank's OE is high at a time (others are Z), so the wired
    bus is safe — no masking or mux logic required.
    """
    out.append("    // -- Output assignments --")
    out.append("    // Each port connects to its slice's shared tri-state wire.")
    out.append("    // Exactly one bank drives at a time; all others are high-Z.")
    out.append("")

    for i, col in enumerate(output_cols):
        bit_idx = n_outputs - 1 - i                        # bit 0 = LSB, bit n_outputs-1 = MSB
        sl      = num_out_slices - 1 - (bit_idx // 64)    # mirrors ROM builder: shift=(num_out_slices-1-sl)*64
        bit_sl  = bit_idx % 64                             # bit position within the 64-bit slice word
        out.append(f"    assign {col} = {dout_shared_wire(sl)}[{bit_sl}];")
...
def generate(csv_path, v_path, module_name):
    ...
    out.append(f"// Architecture: Shared per-slice tri-state wires (dout_s{{sl}}), one assign per output port")
    out.append("")
