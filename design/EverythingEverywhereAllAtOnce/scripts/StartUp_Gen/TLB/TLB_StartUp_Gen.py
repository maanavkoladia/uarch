...
Flat-reg TLB format (TLB_structural.v):
  Each entry's state lives in individually named regs:
    valid_N, present_N, r_w_N, mmio_N, vpn_N, pfn_N
  The JSON config 'paths' should point to the TLB module instance,
  e.g. "uut.core.dc_unit.tlb0"  (NOT .entries -- no struct array).

JSON field names (case-insensitive keys mapped to reg names):
  valid -> valid_N    present -> present_N    r_w -> r_w_N
  MMIO  -> mmio_N     VPN     -> vpn_N        PFN -> pfn_N
"""
...
# Mapping from JSON config key  ->  structural reg name prefix
# (the flat TLB regs are all lower-case; MMIO/VPN/PFN are renamed)
_JSON_TO_REG = {
    "valid":   "valid",
    "present": "present",
    "r_w":     "r_w",
...
def entry_assignments(path: str, idx: int, entry: dict) -> "list[str]":
    ...
    path  – hierarchical path to the TLB *instance*, e.g. dut.u_tlb0
            (NOT the old .entries array path)
    idx   – entry index  (0-7)
    entry – dict with keys: valid, present, r_w, MMIO, VPN, PFN

    Emits flat-reg style:  path.valid_0 = ...  path.vpn_0 = ...
    """
    lines = []

    # 1-bit fields
    for json_key in ("valid", "present", "r_w", "MMIO"):
        reg_name = f"{_JSON_TO_REG[json_key]}_{idx}"
        lines.append(f"{INDENT}{path}.{reg_name} = 1'b{bool_bit(entry[json_key])};")

    # Multi-bit fields – preserve original width/base from JSON
    for json_key in ("VPN", "PFN"):
        reg_name = f"{_JSON_TO_REG[json_key]}_{idx}"
        w, v, b = parse_verilog_literal(entry[json_key])
        lines.append(f"{INDENT}{path}.{reg_name} = {format_literal(w, v, b)};")
...
def generate_verilog(cfg: dict) -> str:
    ...
    while len(paths) < num_tlbs:
        paths.append(f"dut.u_tlb{len(paths)}")
    ...
    # ------------------------------------------------------------------ #
    lines = []
    lines.append(f"// Auto-generated by TLB_StartUp_Gen.py")
