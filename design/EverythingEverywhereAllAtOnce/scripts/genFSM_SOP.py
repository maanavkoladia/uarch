...
with open(sv_path, 'w') as f:
    ...
    # ── File-header comment: state enum + truth table ─────────────────────
    f.write(f"// {'='*70}\n")
    f.write(f"// FSM: {stem}\n")
    f.write(f"// Auto-generated by fsm2rtl.py\n")
    f.write(f"// {'='*70}\n")
    f.write(f"//\n")
    ...
    # Truth table (unexpanded, straight from CSV rows)
    tt_in_hdr  = '  '.join(f"{c:>8}" for c in (state_bit_names + input_cols))
    tt_out_hdr = '  '.join(f"{c:>8}" for c in (ns_bit_names + output_cols))
    sep = '─' * (len(tt_in_hdr) + 4 + len(tt_out_hdr))
    f.write(f"// Truth Table (pre-expansion, from CSV)\n")
    f.write(f"// {sep}\n")
    f.write(f"//  {tt_in_hdr}  |  {tt_out_hdr}\n")
    f.write(f"// {sep}\n")
    for row in rows:
        s_enc_val = state_enc[row[state_col].strip()]
        ns_enc_val = state_enc[row[ns_col].strip()]
        s_bits_str  = format(s_enc_val,  f'0{n_state_bits}b')
        ns_bits_str = format(ns_enc_val, f'0{n_state_bits}b')
        in_bits_str = ''.join(row[c].strip() for c in input_cols)
        out_bits_str = ''.join(
            row[c].strip() if row[c].strip() in ('0','1','-','x','X') else '0'
            for c in output_cols
        )
        # Build columns: one char per state bit, then one per input, | then NS bits, outputs
        in_cols  = list(s_bits_str) + list(in_bits_str)
        out_cols_tt = list(ns_bits_str) + list(out_bits_str)
        in_part  = '  '.join(f"{v:>8}" for v in in_cols)
        out_part = '  '.join(f"{v:>8}" for v in out_cols_tt)
        # Also annotate state names
    ...
    # Current-state output bits (so the state is observable at the module boundary)
    for b in range(n_state_bits):
        port_lines.append(f"    output wire {state_bit_name(b)},  // current-state bit {b}")
    ...
    # ── State / NS wires ───────────────────────────────────────────────────
    f.write("// Current-state bits are output ports (declared in port list above)\n\n")
    for b in range(n_state_bits):
        f.write(f"wire {ns_bit_name(b)};\n")
    f.write("\n")

    # ── State encoding comment ─────────────────────────────────────────────
    f.write("// State encoding (repeated inline for reference)\n")
    for sname, sval in state_enc.items():
        f.write(f"//   {sname:20s} = {format(sval, f'0{n_state_bits}b')} (decimal {sval})\n")
    f.write("\n")
