...
module sram128x8$ (
    ...
    always @(negedge WR or CE)
        if ((CE == 0) && (WR == 0)) begin : write_block
            mem[A] = 'bx;
            ...
            fork
                ...
                forever @(posedge WR or A) disable write_block;
            join
        end
    ...
    always @(addr_unstable_memory) begin
    ...
    always @(CE)
        if ((WR == 1) && (OE == 0) && (CE == 0)) #35 CE_temp = CE;
        else CE_temp = CE;

    always @(A)
        if ((WR == 1) && (OE == 0) && (CE == 0)) #35 A_temp = A;  //t_AA = 35 + t_DOE = 60
        else A_temp = A;
...
module sram32x32$ (
    ...
    always @(CE)
        if ((WR == 1) && (OE == 0) && (CE == 0)) #35 CE_temp = CE;  //t_ACE = 70 + t_DOE = 120 
        else CE_temp = CE;
